...
<div class="seccion izquierda"> 
    <h3> Estructura de la dirección</h3>

<p>Cada dirección de memoria (10 bits) se divide así:</p>

<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Campo</th><th>Símbolo</th><th>Bits</th><th>Descripción</th></tr>
<tr><td>Etiqueta (Tag)</td><td>t</td><td>3</td><td>Identifica qué bloque está almacenado</td></tr>
<tr><td>Índice (Index)</td><td>s</td><td>2</td><td>Indica cuál línea de la caché se usa</td></tr>
<tr><td>Desplazamiento (Offset)</td><td>b</td><td>5</td><td>Indica la posición dentro del bloque</td></tr>
</table>

<p><b>Formato de dirección (10 bits):</b></p>
<pre>[TAG (3 bits)] [INDEX (2 bits)] [OFFSET (5 bits)]</pre>


<h3>Ejemplo con la dirección 0108H</h3>

<p><b>Paso 1. Convertir de hexadecimal a binario</b></p>
<p>\[
0108_H = 0000\,0001\,0000\,1000_2
\]</p>

<p>Solo se toman los 10 bits menos significativos (memoria de 1024 bytes):</p>
<p>\[
0108_H = 0000101000_2
\]</p>

<p><b>Paso 2. Separar según los campos</b></p>
<pre>
TAG (3 bits):       000
ÍNDICE (2 bits):    01
OFFSET (5 bits):    01000
...
<p><b>Paso 3. Convertir a decimal</b></p>

<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Campo</th><th>Valor binario</th><th>Valor decimal</th></tr>
<tr><td>Tag</td><td>000</td><td>0</td></tr>
<tr><td>Índice</td><td>01</td><td>1</td></tr>
<tr><td>Offset</td><td>01000</td><td>8</td></tr>
</table>
...
<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Campo</th><th>Bits</th><th>Valor</th><th>Interpretación</th></tr>
<tr><td>Tag (t)</td><td>3</td><td>000 (0)</td><td>Identifica el bloque en caché</td></tr>
<tr><td>Index (s)</td><td>2</td><td>01 (1)</td><td>Línea de caché número 1</td></tr>
<tr><td>Offset (b)</td><td>5</td><td>01000 (8)</td><td>Byte 8 dentro del bloque</td></tr>
</table>

 
<h3> Estructura de la memoria caché</h3>

<p>Cada línea de la caché contiene:</p>
<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Campo</th><th>Descripción</th></tr>
<tr><td>Bit de validez (V)</td><td>Indica si la línea contiene datos válidos</td></tr>
<tr><td>Etiqueta (Tag)</td><td>Identifica qué bloque de la memoria está almacenado</td></tr>
<tr><td>Datos (Data)</td><td>Contiene el bloque completo (32 bytes)</td></tr>
</table>

<p>Número total de líneas (o conjuntos, porque \(E = 1\)):</p>
<p>\[
S = 4\ \text{líneas (Line 0, Line 1, Line 2, Line 3)}
\]</p>

<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Línea</th><th>Bit de validez</th><th>Etiqueta</th><th>Datos (32 bytes)</th></tr>
<tr><td>0</td><td>V</td><td>Tag</td><td>Bloque de 32 bytes</td></tr>
<tr><td>1</td><td>V</td><td>Tag</td><td>Bloque de 32 bytes</td></tr>
<tr><td>2</td><td>V</td><td>Tag</td><td>Bloque de 32 bytes</td></tr>
<tr><td>3</td><td>V</td><td>Tag</td><td>Bloque de 32 bytes</td></tr>
</table>
...
<ul>
<li>La dirección física tiene <b>10 bits</b>.</li>
<li>Se divide en: <b>3 bits de Tag</b>, <b>2 bits de Índice</b> y <b>5 bits de Offset</b>.</li>
<li>La dirección <b>0108H</b> corresponde a:
  <ul>
    <li>Tag = 000 (0)</li>
    <li>Índice = 01 (1)</li>
    <li>Offset = 01000 (8)</li>
  </ul>
</li>
<li>Por lo tanto, el byte con dirección <b>0108H</b> se encuentra en el <b>bloque 0</b>, <b>línea 1</b> de la caché, y es el <b>byte número 8</b> dentro de ese bloque.</li>
</ul>
 <hr>
...
<table border="1" cellspacing="0" cellpadding="5">
  ...
  <tr>
    <td>\( S = 2^s \)</td>
    <td>Número de conjuntos (sets)</td>
  </tr>
  <tr>
    <td>\( E \)</td>
    <td>Número de líneas por conjunto</td>
  </tr>
  <tr>
    <td>\( B = 2^b \)</td>
    <td>Tamaño del bloque (en bytes)</td>
  </tr>
  <tr>
    <td>\( m = \log_2(M) \)</td>
    <td>Número de bits de dirección de memoria principal (física)</td>
  </tr>
</table>

<h3>Cantidades derivadas</h3>
<table border="1" cellspacing="0" cellpadding="5">
  <tr>
    <th>Parámetro</th>
    <th>Descripción</th>
  ...
  <tr>
    <td>\( M = 2^m \)</td>
    <td>Número máximo de direcciones de memoria únicas</td>
  </tr>
  <tr>
    <td>\( s = \log_2(S) \)</td>
    <td>Número de bits del índice de conjunto</td>
  </tr>
  <tr>
    <td>\( b = \log_2(B) \)</td>
    <td>Número de bits de desplazamiento dentro del bloque</td>
  </tr>
  <tr>
    <td>\( t = m - (s + b) \)</td>
    <td>Número de bits de etiqueta (tag)</td>
  </tr>
  <tr>
    <td>\( C = B \times E \times S \)</td>
    <td>Tamaño total de la caché (en bytes), sin incluir sobrecarga como bits de validez o etiquetas</td>
  </tr>
</table>

<p><strong>Figura 6.28.</strong> Resumen de los parámetros de caché.</p>


<p>Entonces, ¿cómo sabe la caché si contiene una copia de la palabra en la dirección \( A \)?  
La caché está organizada de modo que puede encontrar la palabra solicitada simplemente inspeccionando los bits de la dirección, de manera similar a una tabla hash con una función hash extremadamente simple. Así es como funciona:</p>

<p>Los parámetros \( S \) y \( B \) inducen una partición de los \( m \) bits de dirección en los tres campos mostrados en la Figura 6.27(b).  
Los \( s \) bits del índice de conjunto en \( A \) forman un índice en el arreglo de \( S \) conjuntos.  
El primer conjunto es el conjunto 0, el segundo conjunto es el conjunto 1, y así sucesivamente.  
Cuando se interpretan como un número entero sin signo, los bits del índice de conjunto nos indican en qué conjunto debe almacenarse la palabra.</p>

<p>Una vez que sabemos en qué conjunto debe estar contenida la palabra, los \( t \) bits de etiqueta en \( A \) nos dicen qué línea (si existe) en el conjunto contiene la palabra.  
Una línea en el conjunto contiene la palabra si y solo si el bit de validez está establecido y los bits de etiqueta en la línea coinciden con los bits de etiqueta en la dirección \( A \).  
Una vez que hemos localizado la línea identificada por la etiqueta en el conjunto identificado por el índice de conjunto, entonces los \( b \) bits de desplazamiento de bloque nos dan el desplazamiento de la palabra dentro del bloque de datos de \( B \) bytes.</p>

<p>Como habrás notado, las descripciones de las cachés usan muchos símbolos.  
La Figura 6.28 resume estos símbolos para tu referencia.</p>
</div>
...
<h2> Paso 1. Qué representan los 10 bits</h2>
<p>
Los <b>10 bits</b> (<i>m = 10</i>) pertenecen a la <b>dirección de memoria principal</b>, 
no a lo que se guarda en la caché.
</p>

<p>
Es decir, cada byte en memoria principal tiene una dirección de <b>10 bits</b>, 
porque hay \( 2^{10} = 1024 \) posiciones posibles.
</p>

<p>
Ejemplo: una dirección puede ser <code>0000001010₂</code>, 
que apunta a un byte específico en los 1024 disponibles.
</p>

<hr>

<h2> Paso 2. Cómo se usa esa dirección en la caché</h2>
<p>
Esa dirección <b>se divide en campos</b> para decirle a la caché <b>dónde buscar</b> el dato:
</p>

<p style="font-family: monospace;">
[TAG (3 bits)] [ÍNDICE (2 bits)] [OFFSET (5 bits)]
</p>

<ul>
  <li><b>OFFSET (5 bits)</b> → dice qué byte dentro del bloque se necesita (porque cada bloque tiene 32 bytes).</li>
  <li><b>ÍNDICE (2 bits)</b> → dice <b>en qué línea</b> de las 4 posibles debe buscar.</li>
  <li><b>TAG (3 bits)</b> → sirve para saber <b>qué bloque de memoria</b> está guardado en esa línea.</li>
</ul>

<hr>

<h2> Paso 3. Qué almacena cada línea de caché</h2>
<p>
Cada línea <b>no almacena bits de dirección</b>, sino <b>datos</b> 
(los bytes del bloque) y algunos <b>metadatos</b> (como el tag y el bit de validez).
</p>

<p>Cada línea contiene:</p>
...
<table border="1" cellpadding="5" cellspacing="0">
  ...
  <tr>
    <td><b>Etiqueta (Tag)</b></td>
    <td>Identifica qué bloque de memoria está guardado allí</td>
    <td>3 bits</td>
  </tr>
  <tr>
    <td><b>Bit de validez (V)</b></td>
    <td>Indica si la línea contiene datos válidos</td>
    <td>1 bit</td>
  </tr>
  <tr>
    <td><b>Datos</b></td>
    <td>El bloque traído de memoria</td>
    <td>\( 32 \text{ bytes} = 32 \times 8 = 256 \text{ bits} \)</td>
  </tr>
</table>

<h3> Entonces:</h3>
<p>
Cada línea <b>almacena un bloque completo de 32 bytes (256 bits)</b>, 
además de <b>unos pocos bits extra</b> para el <b>Tag (3 bits)</b> y el <b>bit de validez (1 bit)</b>.
</p>

<hr>
<h2>¿El TAG se repite tanto en la dirección como en cada línea de caché?</h2>

 
 
<h3>1. El TAG en la dirección de memoria</h3>
<p>Cuando la CPU genera una dirección de memoria (por ejemplo, para leer un byte), esa dirección se divide así:</p>

<p>[TAG (3 bits)] [ÍNDICE (2 bits)] [OFFSET (5 bits)]</p>

<p>Esos 3 bits de TAG forman parte de la dirección del bloque.  
No se guardan en la caché directamente en ese momento — simplemente la CPU los usa para identificar a qué bloque 
pertenece la dirección.</p>

<p> En otras palabras, el TAG de la dirección es lo que sale del procesador al acceder a la caché.</p>

<h3>2. El TAG en cada línea de caché</h3>
<p>Cada línea de caché también tiene almacenado su propio TAG (3 bits en este caso).  
Ese TAG fue guardado en el momento en que el bloque de memoria fue cargado en esa línea.  
Sirve para reconocer de qué bloque de memoria provienen los datos almacenados allí.</p>

<h3>3. Cómo se usan juntos</h3>
<ol>
<li>La CPU toma los 2 bits del ÍNDICE → selecciona una línea específica de las 4.</li>
<li>Luego compara el TAG de la dirección (CPU) con el TAG almacenado en esa línea.</li>
<li>Si coinciden y el bit de validez = 1, entonces:<br>
Es un acierto (hit) en caché .  
Se usa el OFFSET (5 bits) para elegir el byte dentro de ese bloque.</li>
<li>Si no coinciden, es un fallo (miss)  y se carga desde memoria principal el bloque correspondiente, actualizando el TAG 
    almacenado.</li>
</ol>
...
<p>Piensa que:</p>
<ul>
<li>El TAG en la dirección es como el nombre del paquete que estás buscando.</li>
<li>El TAG en la línea de caché es el nombre del paquete que ya está guardado allí.</li>
<li>Si el nombre coincide → ya lo tienes (hit).</li>
<li>Si no → hay que reemplazarlo (miss).</li>
</ul>

<h3> Ejemplo</h3>
<p>Supón que la dirección es:</p>
<p>TAG = 011, ÍNDICE = 10, OFFSET = 00101</p>

<p>Y la caché tiene:</p>

<table border="1" cellspacing="0" cellpadding="5">
<tr>
<th>Línea</th>
<th>Validez</th>
<th>TAG almacenado</th>
...
<p>Como el índice es 10₂ = 2, se busca en la línea 2, donde el TAG almacenado también es 011 → coincide con el TAG de la 
    dirección → HIT </p>

<p>El TAG es precisamente el “vínculo” entre la dirección de memoria y la línea de caché donde está guardada esa dirección. Veámoslo con detalle:</p>

<h3>1. Qué hace el TAG</h3>
<p>Cada línea de caché guarda un TAG que representa qué bloque de memoria está almacenado allí.</p>
<p>Cuando la CPU quiere acceder a una dirección \( A \):</p>
<ul>
<li>Usa los bits de índice para elegir el conjunto o la línea dentro de la caché.</li>
<li>Compara el TAG de la dirección con el TAG almacenado en la línea.</li>
</ul>

<h3>2. Condición de acierto (hit)</h3>
<p>Si coinciden los TAGs y el bit de validez = 1 → la caché tiene la palabra correcta.</p>
<p>Entonces se puede acceder al bloque directamente sin ir a memoria principal.</p>

<h3>3. Condición de fallo (miss)</h3>
<p>Si no coinciden los TAGs → la línea de caché no contiene el bloque correcto.</p>
<p>Entonces hay que cargar el bloque de memoria y actualizar el TAG almacenado en la línea.</p>

<h3>4. Conclusión</h3>
<p>Sí, el TAG debe ser el mismo valor:</p>
<ul>
<li>El TAG de la dirección de memoria indica qué bloque estamos buscando.</li>
<li>El TAG almacenado en la línea de caché indica qué bloque está realmente allí.</li>
<li>Solo si coinciden sabemos que la línea corresponde a la dirección deseada.</li>
</ul>
