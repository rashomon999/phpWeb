...
<div class="seccion izquierda"> 
  ...
  <p><strong>Index | Valid | Tag | Data (Hex) | Dirty Bit</strong></p>
...
Index 2 → Set 2</code></pre>
   ...
   <h3>🔴 VALID</h3>
  <p>1 bit. Indica si la línea contiene datos reales o es basura.</p>
  <ul>
    <li><code>0</code> → Línea vacía → MISS automático.</li>
    <li><code>1</code> → Contiene un bloque válido.</li>
  </ul>
  <p>Ejemplo: <code>Valid = 1</code> → los datos y el tag son válidos. <code>Valid = 0</code> → ignora tag y data.</p>

  <h3>🟢 TAG</h3>
  <p>Son los bits altos de la dirección y sirven para comprobar si la línea contiene el bloque correcto.</p>
  <p>Condición para <strong>HIT</strong>:</p>
  <p>\[ (\text{valid} == 1) \;\wedge\; (\text{tag}_{\text{cache}} == \text{tag}_{\text{address}}) \]</p>

  <h3>🟣 DATA (Hex)</h3>
  <p>Es el bloque completo de datos almacenado en esa línea. Contiene \(B\) bytes — típicamente 16 bytes en muchos ejercicios — y se muestra en hexadecimal.</p>
  <pre><code>Data (Hex): 20 00 20 01 20 02 20 03 20 04 ...</code></pre>
  <p>Ese sería el bloque que empieza en la dirección <code class="k">0x200</code>.</p>

  <h3>🟠 DIRTY BIT</h3>
  <p>Solo existe en cachés con política <em>write-back</em>. Indica si el bloque fue modificado por la CPU.</p>
  <ul>
    <li><code>0</code> → limpio (no modificado).</li>
    <li><code>1</code> → sucio (modificado — la memoria principal está desactualizada respecto a la caché).</li>
  </ul>
  <p>En ejercicios básicos de solo lectura muchas veces:</p>
  <ul>
    <li>no aparece, o</li>
    <li>te dicen explícitamente "Ignore Dirty Bit".</li>
  </ul>

  <hr />

  <h2>Ejemplo completo rápido (resumen)</h2>
  <p>Supongamos:</p>
  <ul>
    <li>Tamaño de la dirección: \(m = 11\) bits</li>
    <li>Número de sets: \(S = 4 \Rightarrow s = \log_2(4) = 2\) bits</li>
    <li>Tamaño del bloque: \(B = 16 \Rightarrow b = \log_2(16) = 4\) bits</li>
  </ul>
  <p>Entonces:</p>
  <p>\[ t = m - s - b = 11 - 2 - 4 = 5 \text{ bits (TAG)}. \]</p>

  <p class="field"><strong>Address breakdown</strong> para, por ejemplo, la dirección <code class="k">0x200</code> (representada en binario):</p>
  <pre><code>[ TAG (5 bits) | INDEX (2 bits) | OFFSET (4 bits) ]</code></pre>

  <p>Con esto sabes cómo completar la tabla de la caché en cualquier ejercicio de examen: calcular <code>t</code>, <code>s</code>, <code>b</code>, descomponer la dirección y comprobar valid/tag para decidir HIT o MISS; luego usar OFFSET para elegir el byte dentro del bloque.</p>

  <p style="margin-top:18px; font-size:0.95rem; color:#41536b;">¿Quieres que genere ejemplos tipo examen con varias direcciones y la tabla ya completada paso a paso?</p>

    <hr>

    <header>
  <h1>SIMULACRO DE PARCIAL – ARQUITECTURA DE COMPUTADORES</h1>
  <div class="meta">
    <strong>Nombre:</strong> ____________________ &nbsp;&nbsp;
    <strong>Fecha:</strong> 5 noviembre de 2025<br/>
    <strong>Código:</strong> __________ &nbsp;&nbsp;
    <strong>Duración:</strong> 1.5 horas
  </div>
  <p class="small">
    No se permite el uso de calculadoras graficadoras ni ningún elemento electrónico de almacenamiento. No se permite el uso del celular durante el examen ni dispositivos de audio.
  </p>
</header>

<section>
  <h2>Contexto del experimento</h2>
  <p>El siguiente ejercicio permite el análisis de desempeño de un programa que realiza la transposición parcial de una matriz y la suma de sus elementos en espejo. Este tipo de operación se utiliza en:
    <ul>
      <li>Procesamiento de imágenes (rotaciones, detección de simetrías).</li>
      <li>Machine learning (normalización de pesos en redes neuronales).</li>
      <li>Computación científica (análisis de correlaciones simétricas).</li>
    </ul>
  </p>

  <p>El experimento evalúa el impacto de la localidad de memoria y la política de mapeo en caché sobre dos versiones del mismo programa en C++.</p>

  <h3>Condiciones y supuestos</h3>
  <ul>
    <li>La matriz <strong>A</strong> se almacena en memoria principal de forma consecutiva iniciando en la dirección <code>0x200</code> (200h).</li>
    <li>Los índices y el acumulador (<code>total</code>) se almacenan en registros — para el análisis sólo considere los accesos a la matriz <code>A</code>.</li>
  </ul>

  <h3>Características del sistema (modelo simplificado)</h3>
  <table>
    <tbody>
      <tr><th>Característica</th><th>Valor</th></tr>
      <tr><td>Tamaño de memoria principal</td><td>2 kB</td></tr>
      <tr><td>Latencia de acceso a la DRAM</td><td>20 ns</td></tr>
      <tr><td>Cache (L1)</td><td>64 B</td></tr>
      <tr><td>Latencia de acceso a Cache</td><td>2 ns</td></tr>
      <tr><td>Tamaño de bloque (cache line)</td><td>16 B</td></tr>
      <tr><td>Mapeo cache</td><td>Direct Mapped</td></tr>
      <tr><td>Dirección inicial de matriz A</td><td><code>0x200</code></td></tr>
    </tbody>
  </table>
</section>
...
<section>
  <h2>Representación de la matriz (n = 4)</h2>
  <p>Para ayudar con el análisis, se muestran dos representaciones 4×4 (puedes completar los valores si lo necesitas):</p>

  <div style="margin-bottom:8px;">
    <strong>Matriz A (fila/col índices 0..3)</strong>
    <div class="grid" style="grid-template-columns: repeat(4, 1fr);">
      <div class="matrix-cell">A[0][0]</div>
      <div class="matrix-cell">A[0][1]</div>
      <div class="matrix-cell">A[0][2]</div>
      <div class="matrix-cell">A[0][3]</div>

      <div class="matrix-cell">A[1][0]</div>
      <div class="matrix-cell">A[1][1]</div>
      <div class="matrix-cell">A[1][2]</div>
      <div class="matrix-cell">A[1][3]</div>

      <div class="matrix-cell">A[2][0]</div>
      <div class="matrix-cell">A[2][1]</div>
      <div class="matrix-cell">A[2][2]</div>
      <div class="matrix-cell">A[2][3]</div>

      <div class="matrix-cell">A[3][0]</div>
      <div class="matrix-cell">A[3][1]</div>
      <div class="matrix-cell">A[3][2]</div>
      <div class="matrix-cell">A[3][3]</div>
    </div>
  </div>
</section>

<section>
  <h2>Versiones del programa (C++)</h2>

  <h3>Versión A (original)</h3>
  <pre><code>int A[n][n];
int total = 0;
for (int i = 0; i &lt; n; ++i) {
  for (int k = 0; k &lt; n; ++k) {
...
<section>
  ...
  <div class="question">
    <h3>1.A [20%]</h3>
    <p>Ayudándose con las representaciones de los elementos de la matriz <code>A</code>, indique el orden de acceso a memoria de acuerdo a la versión de algoritmo correspondiente. Determine la complejidad algorítmica de cada versión del programa.</p>
  </div>

  <div class="question">
    <h3>1.B [50%]</h3>
    <p>Como parte de la teoría relevante y resultados esperados del experimento, identifique la ocurrencia de aciertos (<strong>h</strong>) y fallos (<strong>m</strong>) en cada acceso de la matriz. Calcule:</p>
    <ul>
      <li>El total de fallos a la memoria cache.</li>
      <li>La tasa de fallos (Miss Rate).</li>
      <li>El tiempo promedio de acceso (calculado como el tiempo total dividido por la cantidad total de accesos a memoria realizados por cada programa).</li>
    </ul>
    <p>Consigne en la hoja de respuestas todos los cálculos y análisis necesarios, y complete la Tabla #1.</p>
  </div>

  <div class="question">
    <h3>1.C [30%]</h3>
    <p>Se presenta la gráfica del promedio de Tiempo Normalizado vs. Tamaño de Matriz (n) obtenida a partir de un experimento real en un Intel Core i5 comparando las dos versiones (original y unrolling). Los valores representan tiempos normalizados calculados como: <strong>Tiempo Total de Ejecución / n²</strong>.</p>

    <ol>
      <li>
        Con base en la gráfica y en los resultados teóricos obtenidos en la tabla #1 bajo el modelo simplificado:
        <ul>
          <li>Compare las tendencias observadas en la gráfica con los valores de tiempo promedio de acceso calculados teóricamente. ¿Concuerda el experimento con los resultados de la tabla? ¿En qué regiones del gráfico se nota mayor discrepancia entre comportamiento teórico y real?</li>
          <li>Si se asumiera que las latencias proporcionadas (cache y memoria) corresponden a las condiciones reales del experimento, ¿sería razonable comparar directamente el tiempo normalizado con el tiempo promedio de acceso? Justifique.</li>
        </ul>
      </li>
      <li>
        Explique cómo el modelo simplificado (C=64 B=16, direct mapped) limita la interpretación del comportamiento real de un computador moderno (como un Intel Core i5). ¿Qué aspectos importantes deja fuera el modelo? Justifique con argumentos técnicos relacionados con arquitectura.
      </li>
    </ol>
  </div>
</section>
...
<section>
  <h2>Tabla #1</h2>
  <p>Complete la tabla con los resultados del análisis (fallos, miss rate, tiempos).</p>
  <table>
    <thead>
      <tr>
        <th>Versión</th>
        ...
        <th>Miss Rate</th>
        <th>Tiempo total</th>
        <th>Tiempo promedio de acceso</th>
      </tr>
    </thead>
    <tbody>
      <tr><td>Versión 1 (n=4)</td><td> </td><td> </td><td> </td><td> </td></tr>
      <tr><td>Versión 2 (n=4)</td><td> </td><td> </td><td> </td><td> </td></tr>
    </tbody>
  </table>
</section>
