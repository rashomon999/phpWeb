...
<div class="seccion izquierda"> 
  ...
  <p>
    Una <b>DRAM</b> (memoria dinámica) está hecha de <b>celdas</b> organizadas en una <b>matriz</b> 
    (como una tabla con filas y columnas). Cada celda guarda <b>1 bit</b>. 
    Para acceder a una celda, se necesita saber <b>dónde está</b> → eso se hace con <b>direcciones</b>.
  </p>

  <hr>

  <h3> Arreglo lineal</h3>
  <p>
    Si la memoria estuviera organizada <b>en línea recta</b> (una sola fila de celdas), sería algo así:
  </p>

  <pre>[Celda 0] [Celda 1] [Celda 2] ... [Celda 15]</pre>

  <p>
    Aquí, cada celda tiene una dirección única: de <b>0 a 15</b>.<br>
     Para poder elegir entre 16 posiciones diferentes, el chip necesita <b>4 pines de dirección</b> 
    (porque 2⁴ = 16).
  </p>
...
1111 → celda 15
  ...
  <h3> Arreglo bidimensional</h3>
  <p>
    Ahora, si la memoria se organiza como una <b>matriz</b> (por ejemplo, 4 filas × 4 columnas):
  </p>
...
Fila 3   x       x       x       x
  ...
  <p>Ahora tienes <b>dos direcciones separadas</b>:</p>
  <ul>
    <li>una para la <b>fila</b></li>
    <li>otra para la <b>columna</b></li>
  </ul>

  <p>
     Solo necesitas <b>2 pines para la fila</b> (2² = 4 filas) 
    y <b>2 pines para la columna</b> (2² = 4 columnas).<br>
    En total, <b>2 + 2 = 4 pines</b>, pero <b>no se usan al mismo tiempo</b>, 
    sino <b>por turnos</b> (primero fila, luego columna).<br>
    Por eso el chip puede <b>reutilizar los mismos pines</b> para ambos pasos, 
    reduciendo la cantidad <b>física</b> de pines necesarios.<br>
    Así, en lugar de tener <b>4 pines simultáneos</b>, el chip puede tener 
    <b>solo 2 pines</b> que se usan en dos fases.
  </p>

  <hr>

  <h3> Resumen del truco</h3>
  <table border="1" cellspacing="0" cellpadding="5">
    <tr>
      <th>Tipo de organización</th>
      <th>Forma</th>
      <th>Pines necesarios</th>
      <th>Cómo se accede</th>
    </tr>
    <tr>
      <td>Lineal</td>
      <td>16 celdas en fila</td>
      <td>4 pines</td>
      <td>Una sola dirección (0–15)</td>
    </tr>
    <tr>
      <td>Bidimensional</td>
      <td>4×4 matriz</td>
      <td>2 pines reutilizados</td>
      <td>Dos pasos: fila y columna</td>
    </tr>
  </table>
  ...
  <h3> Desventaja</h3>
  <p>
    El <b>acceso toma más tiempo</b>, porque hay que enviar 
    <b>dos direcciones (fila y columna)</b> en lugar de una sola. 
    Primero se selecciona la <b>fila</b>, luego la <b>columna</b>.
  </p>
    <hr>
    <strong>parentesis</strong>
    <h2>¿Qué es un módulo de memoria?</h2>

<p>
Un <strong>módulo de memoria</strong> es una <strong>tarjeta pequeña</strong> que contiene varios 
<strong>chips de memoria DRAM</strong>. Estos módulos son los que tú físicamente ves e insertas 
en los <strong>slots de memoria</strong> de la placa base (motherboard) del computador.
</p>

<p>Ejemplos comunes de módulos:</p>
<ul>
  <li><strong>SIMM</strong> (Single Inline Memory Module): más antiguo, 72 pines, transfiere datos en <strong>bloques de 32 bits</strong>.</li>
  <li><strong>DIMM</strong> (Dual Inline Memory Module): más moderno, 168 pines (o más), transfiere datos en <strong>bloques de 64 bits</strong>.</li>
</ul>

<hr>

<h2>¿Qué hay dentro de un módulo?</h2>

<p>
Dentro de un módulo hay <strong>varios chips DRAM</strong>. Cada chip DRAM contiene miles o millones 
de <strong>superceldas</strong> (grupos de bits organizados en filas y columnas).
</p>

<p>
Por ejemplo:<br>
Un módulo puede tener <strong>8 chips DRAM</strong>, cada uno de <strong>64 megabits (Mbit)</strong>.<br>
En total: 8 × 64 Mbit = <strong>512 Mbit</strong>, que son <strong>64 MB</strong> (megabytes).
</p>

<hr>

<h2>¿Cómo trabajan juntos los chips dentro del módulo?</h2>

<ol>
  <li>La <strong>controladora de memoria</strong> envía una dirección (por ejemplo, A = 2048).</li>
  <li>Esa dirección A se traduce en una <strong>posición dentro del chip</strong>: una fila y una columna, es decir, una <strong>supercelda (i, j)</strong>.</li>
  <li>El módulo de memoria <strong>envía esa dirección (i, j)</strong> a <strong>todos los chips DRAM al mismo tiempo</strong>.</li>
  <li>Cada chip responde con <strong>1 byte (8 bits)</strong> de información.</li>
  <li>Todos esos bytes se combinan para formar una <strong>palabra de 64 bits (8 bytes)</strong>.</li>
</ol>

<p>
Así, cada chip DRAM aporta una parte del dato total.
</p>

<hr>

<h2>Ejemplo gráfico (simplificado)</h2>

<p>
Supón que tenemos un módulo con <strong>8 chips DRAM (0 a 7)</strong>.<br>
Queremos leer el valor en la dirección A.
</p>

<table border="1" cellpadding="4">
  <tr>
    <th>Chip DRAM</th>
    <th>Contenido leído (byte)</th>
    <th>Parte del dato total</th>
  </tr>
  <tr><td>0</td><td>Byte 0</td><td>bits 0–7</td></tr>
  <tr><td>1</td><td>Byte 1</td><td>bits 8–15</td></tr>
  <tr><td>2</td><td>Byte 2</td><td>bits 16–23</td></tr>
  <tr><td>3</td><td>Byte 3</td><td>bits 24–31</td></tr>
  <tr><td>4</td><td>Byte 4</td><td>bits 32–39</td></tr>
  <tr><td>5</td><td>Byte 5</td><td>bits 40–47</td></tr>
  <tr><td>6</td><td>Byte 6</td><td>bits 48–55</td></tr>
  <tr><td>7</td><td>Byte 7</td><td>bits 56–63</td></tr>
</table>

<p>
El módulo los combina y envía a la controladora un <strong>dato de 64 bits (8 bytes)</strong> completo.
</p>
...
<table border="1" cellpadding="4">
  ...
  <tr>
    <td><strong>Módulo de memoria</strong></td>
    <td>Tarjeta que contiene varios chips DRAM.</td>
  </tr>
  <tr>
    <td><strong>Chip DRAM</strong></td>
    <td>Cada uno almacena millones de bits en forma de celdas o superceldas.</td>
  </tr>
  <tr>
    <td><strong>Supercelda</strong></td>
    <td>Grupo de bits (por ejemplo, 8 bits = 1 byte) dentro del chip.</td>
  </tr>
  <tr>
    <td><strong>Controladora de memoria</strong></td>
    <td>Envía direcciones a los chips y coordina la lectura o escritura.</td>
  </tr>
  <tr>
    <td><strong>Ancho de bus de datos</strong></td>
    <td>Cantidad de bits que se transfieren a la vez (32 bits en SIMM, 64 bits en DIMM).</td>
  </tr>
</table>
    <hr>
</div>
...
<div class="seccion derecha">
   ...
   <h3>6.4.1 Organización Genérica de Memoria Caché</h3>
<p>
Considere un sistema de computadora donde cada dirección de memoria tiene <i>m</i> bits que forman
<i>M = 2<sup>m</sup></i> direcciones únicas. Como se ilustra en la Figura 6.27(a), una caché para tal máquina se organiza como un arreglo de <i>S = 2<sup>s</sup></i> conjuntos de caché. Cada conjunto consiste en <i>E</i> líneas de caché. Cada línea consiste en un bloque de datos de <i>B = 2<sup>b</sup></i> bytes, un bit válido que indica si la línea contiene o no información significativa, y <i>t = m − (b + s)</i> bits de etiqueta (un subconjunto de los bits de la dirección de memoria del bloque actual) que identifican de manera única el bloque almacenado en la línea de caché.
</p>
<p>
En general, la organización de una caché puede caracterizarse por la tupla (<i>S, E, B, m</i>). El tamaño (o capacidad) de una caché, <i>C</i>, se expresa en términos del tamaño total de todos los bloques. Los bits de etiqueta y el bit válido no se incluyen. Así,
<i>C = S × E × B.</i>
</p>
<p>
Cuando la CPU recibe la instrucción de carga para leer una palabra desde la dirección <i>A</i> de la memoria principal, envía la dirección <i>A</i> a la caché. Si la caché tiene una copia de la palabra en la dirección <i>A</i>, envía la palabra inmediatamente de vuelta a la CPU.
</p>
<p>
<b>Figura 6.27</b><br>
Organización general de la caché (<i>S, E, B, m</i>).<br>
(a) Una caché es un arreglo de conjuntos. Cada conjunto contiene una o más líneas. Cada línea contiene un bit válido, algunos bits de etiqueta y un bloque de datos.<br>
(b) La organización de la caché induce una partición de los <i>m</i> bits de dirección en <i>t</i> bits de etiqueta, <i>s</i> bits de índice de conjunto y <i>b</i> bits de desplazamiento de bloque.
</p>

    <img src="../../img/guia_282.png" alt="">

  

<h2> PROBLEMA 1 — Mapeo Directo en Memoria Caché</h2>

<p>
Se tiene un sistema con una <b>memoria principal de 1024 bytes (1 KB)</b>, donde cada bloque tiene un tamaño de <b>32 bytes</b>.
La <b>memoria caché</b> es de <b>correspondencia directa (direct-mapped)</b> con una <b>capacidad total de 128 bytes</b>.
</p>

<ol>
<li>Dibuje o describa el <b>esquema de la memoria caché</b>, indicando:
  <ul>
    <li>El número total de líneas o conjuntos.</li>
    <li>El tamaño de cada línea.</li>
    <li>Los campos que componen cada entrada (Etiqueta, Bit de Validez y Datos).</li>
  </ul>
</li>
<li>Determine cómo se <b>divide la dirección de memoria física</b> (en bits de etiqueta, índice y desplazamiento).</li>
<li>Para la <b>dirección 0108H</b>, indique el <b>tag</b>, <b>índice</b> y <b>offset</b> que le corresponden.</li>
</ol>
...
<p>
En una memoria caché, los parámetros fundamentales definen su estructura y la forma en que las direcciones de memoria se traducen a posiciones en caché.
Uno de los parámetros más importantes es <b>S</b>, que representa el <b>número de conjuntos (sets)</b> en la caché.
Cada conjunto puede contener una o más líneas de datos (<b>E</b> líneas por conjunto).
</p>

<p>En general, se tienen los siguientes parámetros:</p>

<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Símbolo</th><th>Descripción</th><th>Unidades</th></tr>
<tr><td>C</td><td>Tamaño total de la caché</td><td>bytes</td></tr>
<tr><td>B</td><td>Tamaño del bloque (o línea)</td><td>bytes</td></tr>
<tr><td>E</td><td>Número de líneas por conjunto</td><td>líneas</td></tr>
<tr><td>S</td><td>Número de conjuntos en la caché</td><td>conjuntos</td></tr>
<tr><td>M</td><td>Tamaño de la memoria principal</td><td>bytes</td></tr>
<tr><td>m</td><td>Número total de bits en la dirección física</td><td>bits</td></tr>
</table>

<p>
Para una <b>caché de mapeo directo</b>, se cumple que \(E = 1\),
por lo tanto, el número de conjuntos \(S\) coincide con el número total de líneas de la caché.
</p>

<hr>

<h3> Notaciones y fórmulas</h3>

<p><b>Parámetros fundamentales</b></p>

<p>
\[
\begin{aligned}
S &= \frac{C}{B \times E} &&\text{Número de conjuntos} \\
B &= 2^{b} &&\text{Tamaño de bloque en bytes} \\
M &= 2^{m} &&\text{Tamaño total de memoria principal}
\end{aligned}
\]
</p>

<p><b>Parámetros derivados</b></p>

<p>
\[
\begin{aligned}
s &= \log_{2}(S) &&\text{Bits de índice de conjunto} \\
b &= \log_{2}(B) &&\text{Bits de desplazamiento (offset)} \\
t &= m - (s + b) &&\text{Bits de etiqueta (tag)} \\
C &= B \times E \times S &&\text{Tamaño total de caché en bytes}
\end{aligned}
\]
</p>
...
<table border="1" cellspacing="0" cellpadding="4">
<tr><th>Parámetro</th><th>Descripción</th><th>Valor</th></tr>
<tr><td>\(M = 1024\ \text{bytes}\)</td><td>Tamaño de memoria principal</td><td>\(2^{10}\)</td></tr>
<tr><td>\(B = 32\ \text{bytes}\)</td><td>Tamaño de bloque</td><td>\(2^{5}\)</td></tr>
<tr><td>\(C = 128\ \text{bytes}\)</td><td>Tamaño total de la caché</td><td>—</td></tr>
<tr><td>Tipo</td><td>Correspondencia directa</td><td>\(E = 1\)</td></tr>
</table>
...
<p><b> Número de conjuntos (S)</b></p>
<p>\[
S = \frac{C}{B \times E} = \frac{128}{32 \times 1} = 4
\]</p>

<p><b> Bits de dirección total (m)</b></p>
<p>\[
m = \log_{2}(M) = \log_{2}(1024) = 10\ \text{bits}
\]</p>

<p><b> Bits de desplazamiento (b)</b></p>
<p>\[
b = \log_{2}(B) = \log_{2}(32) = 5\ \text{bits}
\]</p>

<p><b> Bits de índice de conjunto (s)</b></p>
<p>\[
s = \log_{2}(S) = \log_{2}(4) = 2\ \text{bits}
\]</p>

<p><b> Bits de etiqueta (t)</b></p>
<p>\[
t = m - (s + b) = 10 - (2 + 5) = 3\ \text{bits}
\]</p>
