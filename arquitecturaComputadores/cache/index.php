...
<div class="seccion izquierda"> 
  ...
  <p>
    Hasta este punto en nuestro estudio de los sistemas, hemos confiado en un modelo simple de un sistema
    computacional como una CPU que ejecuta instrucciones y un sistema de memoria que contiene las instrucciones
    y los datos para la CPU. En nuestro modelo simple, el sistema de memoria es un arreglo lineal de bytes, y la CPU
    puede acceder a cada ubicación de memoria en una cantidad constante de tiempo. Aunque este es un modelo
    efectivo hasta cierto punto, no refleja la manera en que realmente funcionan los sistemas modernos.
  </p>

  <p>
    En la práctica, un sistema de memoria es una jerarquía de dispositivos de almacenamiento con diferentes
    capacidades, costos y tiempos de acceso. Los registros de la CPU contienen los datos que se usan con mayor
    frecuencia. Pequeñas y rápidas memorias caché cercanas a la CPU actúan como áreas intermedias para un
    subconjunto de los datos e instrucciones almacenados en la relativamente lenta memoria principal. La memoria
    principal prepara los datos almacenados en discos grandes y lentos, los cuales, a su vez, a menudo sirven como
    áreas intermedias para datos almacenados en los discos o cintas de otras máquinas conectadas por redes.
  </p>

  <p>
    Las jerarquías de memoria funcionan porque los programas bien escritos tienden a acceder al almacenamiento en
    un nivel particular con más frecuencia que al almacenamiento en el siguiente nivel inferior. Así, el almacenamiento
    en el siguiente nivel puede ser más lento y, por lo tanto, más grande y barato por bit. El efecto general es un gran
    conjunto de memoria que cuesta tanto como el almacenamiento barato cerca de la parte inferior de la jerarquía,
    pero que entrega los datos a los programas a la velocidad del almacenamiento rápido cerca de la parte superior
    de la jerarquía.
  </p>

  <p>
    Como programador, necesitas entender la jerarquía de memoria porque tiene un gran impacto en el rendimiento
    de tus aplicaciones. Si los datos que tu programa necesita están almacenados en un registro de la CPU, entonces
    pueden ser accedidos en cero ciclos durante la ejecución de la instrucción. Si están almacenados en la caché, de 1
    a 30 ciclos. Si están en la memoria principal, de 50 a 200 ciclos. ¡Y si están en el disco, decenas de millones de ciclos!
  </p>

  <p>
    Aquí, entonces, hay una idea fundamental y duradera en los sistemas computacionales: si entiendes cómo el sistema
    mueve los datos hacia arriba y hacia abajo en la jerarquía de memoria, entonces puedes escribir tus programas de
    aplicación de manera que sus elementos de datos estén almacenados más arriba en la jerarquía, donde la CPU pueda
    acceder a ellos más rápidamente.
  </p>

  <p>
    Esta idea gira en torno a una propiedad fundamental de los programas de computadora conocida como localidad.
    Los programas con buena localidad tienden a acceder al mismo conjunto de elementos de datos una y otra vez, o
    tienden a acceder a conjuntos de elementos de datos cercanos. Los programas con buena localidad tienden a acceder
    a más elementos de datos de los niveles superiores de la jerarquía de memoria que los programas con poca localidad,
    y por lo tanto se ejecutan más rápido. Por ejemplo, los tiempos de ejecución de diferentes núcleos de multiplicación
    de matrices que realizan el mismo número de operaciones aritméticas, pero tienen diferentes grados de localidad,
    pueden variar por un factor de 20.
  </p>

  <p>
    En este capítulo, veremos las tecnologías básicas de almacenamiento: memoria SRAM, memoria DRAM, memoria
    ROM y discos rotatorios y de estado sólido, y describiremos cómo se organizan en jerarquías. En particular, nos
    enfocamos en las memorias caché que actúan como áreas intermedias entre la CPU y la memoria principal, porque
    tienen el mayor impacto en el rendimiento de los programas de aplicación. Te mostraremos cómo analizar tus
    programas en C para la localidad y te presentaremos técnicas para mejorar la localidad en tus programas. También
    aprenderás una forma interesante de caracterizar el rendimiento de la jerarquía de memoria en una máquina en
    particular como una "montaña de memoria" que muestra los tiempos de acceso de lectura como una función de la
    localidad.
  </p>

  <h2>6.1 Tecnologías de almacenamiento</h2>

  <p>
    Gran parte del éxito de la tecnología informática proviene del tremendo progreso en la tecnología de almacenamiento.
    Las primeras computadoras tenían unos pocos kilobytes de memoria de acceso aleatorio. Las primeras PC de IBM ni
    siquiera tenían un disco duro. Eso cambió con la introducción de la IBM PC-XT en 1982, con su disco de 10 megabytes.
    Para el año 2010, las máquinas típicas tenían 150,000 veces más almacenamiento en disco, y la cantidad de
    almacenamiento estaba aumentando por un factor de 2 cada par de años.
  </p>

  <h3>6.1.1 Memoria de acceso aleatorio</h3>

  <p>
    La memoria de acceso aleatorio (RAM) viene en dos variedades: estática y dinámica. La RAM estática (SRAM) es más
    rápida y significativamente más costosa que la RAM dinámica (DRAM). La SRAM se usa para memorias caché, tanto
    dentro como fuera del chip de la CPU. La DRAM se usa para la memoria principal y el búfer de cuadros de un sistema
    gráfico. Típicamente, un sistema de escritorio tendrá no más de unos pocos megabytes de SRAM, pero cientos o miles
    de megabytes de DRAM.
  </p>

  <h4>Memoria RAM estática</h4>

  <p>
    La SRAM almacena cada bit en una celda de memoria biestable. Cada celda se implementa con un circuito de seis
    transistores. Este circuito tiene la propiedad de que puede permanecer indefinidamente en una de dos diferentes
    configuraciones de voltaje, o estados. Cualquier otro estado será inestable; comenzando desde allí, el circuito se moverá
    rápidamente hacia uno de los estados estables. Tal celda de memoria es análoga al péndulo invertido ilustrado en la
    Figura 6.1.
  </p>

  <p>
    El péndulo es estable cuando está inclinado completamente hacia la izquierda o completamente hacia la derecha.
    Desde cualquier otra posición, el péndulo caerá hacia un lado u otro. En principio, el péndulo también podría permanecer
    equilibrado en una posición vertical indefinidamente, pero este estado es metaestable: la más pequeña perturbación haría
    que comenzara a caer, y una vez que cae nunca regresaría a la posición vertical.
  </p>

  <p>
    Debido a su naturaleza biestable, una celda de memoria SRAM conservará su valor indefinidamente, siempre que se
    mantenga alimentada. Incluso cuando una perturbación, como el ruido eléctrico, altera los voltajes, el circuito regresará
    al valor estable cuando la perturbación sea eliminada.
  </p>

  <h4>Memoria RAM dinámica</h4>

  <p>
    La DRAM almacena cada bit como una carga en un condensador (capacitor). Este condensador es muy pequeño—típicamente
    alrededor de 30 femtofaradios, es decir, 30 × 10⁻¹⁵ faradios. Sin embargo, recuerda que un faradio es una unidad de
    medida muy grande. El almacenamiento DRAM puede hacerse muy denso—cada celda consiste en un condensador y
    un solo transistor de acceso. A diferencia de la SRAM, sin embargo, una celda de memoria DRAM es muy sensible a
    cualquier perturbación. Cuando el voltaje del condensador se ve alterado, nunca se recuperará. La exposición a rayos
    de luz hará que los voltajes del condensador cambien. De hecho, los sensores en las cámaras digitales y videocámaras
    son esencialmente arreglos de celdas DRAM.
  </p>

  <p>
    Diversas fuentes de corriente de fuga hacen que una celda DRAM pierda su carga dentro de un período de tiempo de
    alrededor de 10 a 100 milisegundos. Afortunadamente, para las computadoras que operan con tiempos de ciclo de reloj
    medidos en nanosegundos, este tiempo de retención es bastante largo. El sistema de memoria debe refrescar
    periódicamente cada bit de memoria leyéndolo y luego reescribiéndolo. Algunos sistemas también usan códigos de
    corrección de errores, donde las palabras del computador se codifican con unos pocos bits adicionales (por ejemplo,
    una palabra de 32 bits podría codificarse usando 38 bits), de modo que el circuito pueda detectar y corregir cualquier bit
    erróneo dentro de una palabra.
  </p>

  <p>
    La Figura 6.2 resume las características de la memoria SRAM y DRAM. La SRAM es persistente mientras se mantenga
    la alimentación eléctrica. A diferencia de la DRAM, no es necesario el refresco. La SRAM puede ser accedida más
    rápidamente que la DRAM. La SRAM no es sensible a perturbaciones como la luz y el ruido eléctrico. La compensación
    es que las celdas SRAM usan más transistores que las celdas DRAM y, por lo tanto, tienen menor densidad, son más
    costosas y consumen más energía.
  </p>
  ...
  <table border="1" cellspacing="0" cellpadding="5">
    <thead>
      <tr>
        <th></th>
        <th>Transistores por bit</th>
        <th>Tiempo de acceso relativo</th>
        <th>¿Persistente?</th>
        <th>¿Sensible?</th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Sí</td>
        <td>1×</td>
        <td>Memoria principal, búferes de cuadros</td>
      </tr>
    </tbody>
  </table>

  <p><strong>Figura 6.2</strong> Características de la memoria DRAM y SRAM.</p>
...
<div class="seccion derecha">
     ...
     <h4>DRAM Convencional</h4>

  <p>
    Las celdas (bits) en un chip DRAM están divididas en <i>d</i> superceldas, cada una compuesta por <i>w</i> celdas DRAM.
    Una DRAM de <i>d × w</i> almacena un total de <i>dw</i> bits de información. Las superceldas están organizadas como un
    arreglo rectangular con <i>r</i> filas y <i>c</i> columnas, donde <i>r × c = d</i>. Cada supercelda tiene una dirección de la forma
    (<i>i, j</i>), donde <i>i</i> representa la fila y <i>j</i> representa la columna.
  </p>

  <p>
    Por ejemplo, la Figura 6.3 muestra la organización de un chip DRAM de <i>16 × 8</i> con <i>d = 16</i> superceldas,
    <i>w = 8</i> bits por supercelda, <i>r = 4</i> filas y <i>c = 4</i> columnas. El recuadro sombreado indica la supercelda con dirección
    (<i>2, 1</i>). La información fluye hacia dentro y hacia fuera del chip a través de conectores externos llamados pines.
    Cada pin transporta una señal de 1 bit. La Figura 6.3 muestra dos de estos conjuntos de pines: ocho pines de datos
    que pueden transferir 1 byte dentro o fuera del chip, y dos pines de dirección que transportan direcciones de superceldas
    de fila y columna de dos bits. No se muestran otros pines que transportan información de control.
  </p>

  <h5>Nota sobre la terminología</h5>

  <p>
    La comunidad de almacenamiento nunca ha acordado un nombre estándar para un elemento de arreglo DRAM.
    Los arquitectos de computadoras tienden a referirse a él como una “celda”, sobrecargando el término con la celda
    de almacenamiento DRAM. Los diseñadores de circuitos tienden a referirse a él como una “palabra”, sobrecargando
    el término con una palabra de la memoria principal. Para evitar confusión, hemos adoptado el término inequívoco
    “supercelda”.
  </p>

  <p>
    Cada chip DRAM está conectado a cierto circuito, conocido como el controlador de memoria, que puede transferir
    <i>w</i> bits a la vez hacia y desde cada chip DRAM. Para leer el contenido de la supercelda (<i>i, j</i>), el controlador de memoria
    envía la dirección de fila <i>i</i> a la DRAM, seguida de la dirección de columna <i>j</i>. La DRAM responde enviando el contenido
    de la supercelda (<i>i, j</i>) de vuelta al controlador. La dirección de fila <i>i</i> se denomina solicitud <b>RAS</b>
    (<i>Row Access Strobe</i>). La dirección de columna <i>j</i> se denomina solicitud <b>CAS</b> (<i>Column Access Strobe</i>).
    Nótese que las solicitudes RAS y CAS comparten los mismos pines de dirección de la DRAM.
  </p>

  <p>
    Por ejemplo, para leer la supercelda (<i>2, 1</i>) de la DRAM de <i>16 × 8</i> mostrada en la Figura 6.3, el controlador de memoria
    envía la dirección de fila 2, como se muestra en la Figura 6.4(a). La DRAM responde copiando todo el contenido de la
    fila 2 en un búfer de fila interno. Luego, el controlador de memoria envía la dirección de columna 1, como se muestra
    en la Figura 6.4(b). La DRAM responde copiando los 8 bits de la supercelda (<i>2, 1</i>) desde el búfer de fila y enviándolos
    al controlador de memoria.
  </p>
     <img src="../../img/guia_280.png" alt="">
    <hr>
    <strong>parentesis</strong>
    <h3>💾 En cada tipo de memoria:</h3>

  <h4>En SRAM (Static RAM)</h4>
  <p>
    Cada celda está formada por 6 transistores que pueden mantener el valor del bit mientras haya energía.
  </p>

  <h4>En DRAM (Dynamic RAM)</h4>
  <p>
    Cada celda está formada por 1 transistor y 1 condensador.
  </p>
  <p>
    El condensador guarda una pequeña carga eléctrica que representa el bit, pero esa carga se fuga con el tiempo,
    por eso la memoria debe refrescarse constantemente (volver a leer y escribir los bits para que no se pierdan).
  </p>

  <h4>🧩 Sobre las superceldas</h4>
  <p>
    El autor luego habla de “superceldas”, que son grupos de varias celdas de memoria (por ejemplo, 8 bits = 1 byte)
    organizadas en una especie de matriz (filas y columnas) dentro del chip DRAM.
  </p>
    <hr>


    <p>
    Una razón por la que los diseñadores de circuitos organizan las DRAM como arreglos bidimensionales
    en lugar de arreglos lineales es para reducir el número de pines de dirección en el chip.
    Por ejemplo, si nuestra DRAM de 128 bits de ejemplo estuviera organizada como un arreglo lineal de 16
    superceldas con direcciones de 0 a 15, entonces el chip necesitaría cuatro pines de dirección
    en lugar de dos. La desventaja de la organización en arreglo bidimensional es que las direcciones deben enviarse
    en dos pasos distintos, lo que aumenta el tiempo de acceso.
  </p>

  <h3>Modulos de Memoria</h3>

  <p>
    Los chips DRAM se empaquetan en módulos de memoria que se conectan en ranuras de expansión
    en la placa base principal (motherboard). Los paquetes comunes incluyen el módulo de memoria
    de doble línea de 168 pines (DIMM), que transfiere datos hacia y desde el controlador de memoria
    en bloques de 64 bits, y el módulo de memoria de línea simple de 72 pines (SIMM),
    que transfiere datos en bloques de 32 bits.
  </p>

  <p>
    La Figura 6.5 muestra la idea básica de un módulo de memoria.
    El módulo de ejemplo almacena un total de 64 MB (megabytes) usando ocho chips DRAM de 64 Mbit
    de 8M × 8, numerados del 0 al 7. Cada supercelda almacena 1 byte de la memoria principal,
    y cada palabra doble de 64 bits en la dirección de byte A en la memoria principal está representada
    por las ocho superceldas cuya dirección de supercelda correspondiente es (i, j).
    En el ejemplo de la Figura 6.5, la DRAM 0 almacena el primer byte (de menor orden),
    la DRAM 1 almacena el siguiente byte, y así sucesivamente.
  </p>

  <p>
    Para recuperar una palabra doble de 64 bits en la dirección de memoria A,
    el controlador de memoria convierte A en una dirección de supercelda (i, j)
    y la envía al módulo de memoria, que luego transmite i y j a cada DRAM.
    En respuesta, cada DRAM produce el contenido de 8 bits de su supercelda (i, j).
    Los circuitos en el módulo recogen estas salidas y las forman en una palabra doble de 64 bits,
    que devuelve al controlador de memoria.
  </p>

  <p>
    La memoria principal puede agregarse conectando múltiples módulos de memoria al controlador de memoria.
    En este caso, cuando el controlador recibe una dirección A, el controlador selecciona el módulo k
    que contiene A, convierte A en su forma (i, j), y envía (i, j) al módulo k.
  </p>

   <img src="../../img/guia_281.png" alt="">
</div>
</div>
 </form>
