...
<h1>¿Qué es <strong>stride</strong>?</h1>
<p><strong>Stride</strong> (en computación, especialmente en acceso a memoria) es <strong>el salto o distancia entre accesos consecutivos a memoria</strong> dentro de un programa.</p>


<h2>Ejemplos</h2>
<p>Acceso contiguo (stride = 1):</p>
<pre><code>// C
for (int i = 0; i &lt; n; i++) {
x = A[i];
}
...
<ul>
<li><strong>Stride bajo</strong> (accesos contiguos) =&gt; mejor desempeño: aprovecha la localidad espacial de la caché.</li>
<li><strong>Stride alto</strong> (accesos con grandes saltos) =&gt; peor desempeño: menos reutilización de líneas de caché y más <em>cache misses</em>.</li>
</ul>


<h2>Pregunta</h2>
<p><em>Los factores que pueden afectar negativamente el desempeño de un programa pueden ser:</em></p>


<div class="options">
<div class="option">a. tamaño de memoria principal muy grande y un stride muy bajo</div>
<div class="option">b. El tamaño de cache muy grande y un stride muy alto</div>
<div class="option">c. Una velocidad de reloj de la CPU muy baja y un stride muy bajo</div>
<div class="option correct">d. <strong>Una memoria cache con una capacidad muy pequeña y un stride muy alto</strong></div>
</div>
...
<p class="explain">Esta combinación afecta negativamente el desempeño porque una caché con poca capacidad no puede retener suficientes datos y un stride alto provoca accesos que saltan mucho, reduciendo la localidad espacial y aumentando los <em>cache misses</em>.</p>
</div>
</div>
...
<form>
 ...
 <h1>✅ Por qué la respuesta correcta es la <strong>(a)</strong></h1>
<p class="lead">La opción (a) dice:</p>
<blockquote>
<strong>Se accede continuamente a la memoria principal y con ello se accede a una gran cantidad de datos aprovechando la localidad temporal.</strong>
</blockquote>
<p>El problema es que esta frase mezcla algo <strong>bueno</strong> con algo <strong>malo</strong>, y por eso confunde.</p>


<h2>🔍 Interpretación técnica correcta</h2>
<h3>✔ Lo bueno (lo que confunde)</h3>
<p>La frase dice “aprovechando la localidad temporal”. Eso <strong>suena positivo</strong>.</p>


<h3>❌ Lo malo (lo importante en la pregunta)</h3>
<p>El inicio dice:</p>
<blockquote>
<strong>Se accede continuamente a la memoria principal</strong>
</blockquote>
<p>Esto <strong>sí es negativo</strong>, porque:</p>
<ul>
<li>La <strong>memoria principal</strong> (RAM) es mucho más lenta que la caché.</li>
<li>Acceder continuamente a RAM implica: más latencia, más ciclos perdidos y menor rendimiento.</li>
</ul>
<p class="note"><strong>Nota:</strong> un acceso a memoria principal puede ser órdenes de magnitud más lento que un acceso a L1; por eso, aunque exista localidad temporal, si los datos no residen en caché, el rendimiento empeora.</p>


<h2>📌 ¿Por qué NO son correctas las opciones b, c, d, e?</h2>
<p>Todas describen factores que <strong>mejoran</strong> el rendimiento:</p>
<ul>
<li><strong>b)</strong> stride = 1 → buena localidad espacial → mejora.</li>
<li><strong>c)</strong> buena localidad temporal → mejora.</li>
<li><strong>d)</strong> varios niveles de caché → mejora.</li>
<li><strong>e)</strong> más cache hits → mejora.</li>
</ul>
...
<table>
<thead>
<tr><th>Opción</th><th>¿Mejora o empeora?</th><th>Motivo</th></tr>
</thead>
<tbody>
<tr><td><strong>a</strong></td><td class="bad">Empeora</td><td>Acceso continuo a memoria principal = muy lento</td></tr>
<tr><td><strong>b</strong></td><td class="ok">Mejora</td><td>stride=1 = buena localidad espacial</td></tr>
<tr><td><strong>c</strong></td><td class="ok">Mejora</td><td>buena localidad temporal</td></tr>
<tr><td><strong>d</strong></td><td class="ok">Mejora</td><td>más niveles de cache = más rápido</td></tr>
<tr><td><strong>e</strong></td><td class="ok">Mejora</td><td>más cache hits = mejor</td></tr>
</tbody>
</table>
...
<p>Porque <strong>acceder continuamente a memoria principal</strong> es exactamente lo que <em>afecta negativamente el desempeño</em> (aumenta la latencia y el tiempo de ejecución).</p>
...
<div class="seccion derecha">
 ...
 <h1>DRAM vs SRAM — explicación breve</h1>


<p>La afirmación original:</p>
<blockquote>
“Las memorias DRAM se construyen con flip-flops y son más rápidas que las memorias SRAM.”
</blockquote>


<p>Esta afirmación es <strong class="bad">incorrecta</strong> por dos razones clave:</p>


<h2>❌ 1. La DRAM no usa flip-flops</h2>
<p>La <strong>SRAM</strong> sí utiliza flip-flops (celdas de 6 transistores por bit en implementaciones típicas).<br>
La <strong>DRAM</strong> se implementa con <strong>capacitores + un transistor</strong> (1T + 1C), no con flip-flops.</p>


<h2>❌ 2. La DRAM NO es más rápida que la SRAM</h2>
<p>La <strong>SRAM</strong> es mucho más rápida, por eso se usa como memoria caché. La <strong>DRAM</strong> es más lenta, pero ofrece mayor densidad y menor costo por bit, por eso se usa como memoria principal (RAM).</p>


<p class="badge">Respuesta correcta:</p>
...
<h2>✔️ Flip-flops y número de transistores</h2>
<h3>1) Flip-flop lógico estándar</h3>
<p>Un flip-flop básico (núcleo biestable, por ejemplo dos inversores cruzados) se construye con:</p>
<ul>
<li>2 transistores por inversor</li>
<li>2 inversores cruzados</li>
</ul>
<p><strong>Total:</strong> 2 + 2 = <strong>4 transistores</strong></p>


<pre>// Núcleo biestable (conceptual)
2 transistores (inversor A)
2 transistores (inversor B)
= 4 transistores (flip-flop básico)
</pre>


<h3>2) Celda de memoria SRAM (6T)</h3>
<p>En memoria SRAM la celda típica añade <strong>2 transistores de acceso</strong> al flip-flop para permitir lectura/escritura desde las bitlines.</p>
<p><strong>Desglose:</strong></p>
<ul>
<li>4 transistores → forman el flip-flop (estado estable)</li>
<li>+ 2 transistores → transistores de acceso (leer/escribir)</li>
</ul>


<pre>4 (flip-flop) + 2 (acceso) = 6 transistores → celda SRAM (6T)</pre>


<h2>🧠 Resumen claro</h2>
<table>
<thead>
<tr><th>Contexto</th><th>Nº transistores</th><th>Explicación</th></tr>
</thead>
<tbody>
<tr><td>Flip-flop lógico estándar</td><td>4</td><td>Dos inversores cruzados forman el núcleo biestable</td></tr>
<tr><td>Celda de memoria SRAM</td><td>6</td><td>4 para el biestable + 2 de acceso para leer/escribir</td></tr>
</tbody>
</table>
<hr>

    <p>
    <strong>M =</strong>  Memoria principal (RAM).
    </p>
    
    <p>
    <strong>m = </strong>
    es la cantidad de bits que vamos a necesitar para representar cada una de las direcciones en la memoria principal    
    </p>
    ...
    <p>
    Una dirección es simplemente un número (en binario o hexadecimal) que identifica un byte dentro de la memoria principal.<br>
    Ejemplo: <code class="k">0x200</code> → dirección en memoria.
    </p>

    <p>
    La caché no cambia la dirección, sino que la <strong>divide</strong> según:
    </p>
  ...
  <p>Esto se llama <em>Address Breakdown</em> y depende del tamaño de la caché y del bloque.</p>

  <h2>2) ¿Qué representa cada parte de la dirección?</h2>

  <h3>🔵 OFFSET (selecciona el byte dentro del bloque)</h3>
  ...
  <p>El INDEX es la parte de la dirección que identifica la línea (o set) específica de la caché donde el bloque de memoria debe almacenarse o buscarse.</p>
  <p>\[ s = \log_{2}(S) \]</p>
  ...
  <p>\[ t = m - s - b \]</p>
  <p>El TAG es la parte alta de la dirección que identifica de qué bloque de memoria proviene el dato que está almacenado en una línea de caché.</p>
