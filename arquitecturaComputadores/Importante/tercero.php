...
<strong>Duda: abajo de muchas piramides abajo de cache esta RAM, porque?</strong>

<h3> 1. Tipos físicos de memoria (SRAM vs DRAM)</h3>
<h3> 2. Niveles funcionales de la jerarquía de memoria (caché, RAM, disco, etc.)</h3>

<p>Vamos a separar ambas para que se entienda bien:</p>

<h3> 1. Jerarquía funcional (lo que muestran las pirámides)</h3>
<p>Esto muestra qué tan rápido y accesible es cada nivel para el procesador. De arriba a abajo (de más rápida a más lenta):</p>
<p>
  Registros del CPU<br>
  ↓<br>
  <strong>Memoria caché</strong> (usa SRAM)<br>
  ↓<br>
  <strong>Memoria principal (RAM)</strong> – (usa DRAM)<br>
  ↓<br>
  <strong>Almacenamiento secundario</strong> (SSD, HDD)<br>
  ↓<br>
  <strong>Almacenamiento terciario</strong> (nube, cinta, etc.)
</p>

<p> Aquí “caché encima de RAM” está correcto funcionalmente, porque el procesador accede primero a la caché, y si no encuentra los datos allí, va a la RAM.</p>

<h3> 2. Tipos físicos de RAM</h3>
<p>A nivel técnico de construcción de chips:</p>
<ul>
  <li><strong>SRAM</strong> = usada en caché.</li>
  <li><strong>DRAM</strong> = usada en la RAM principal.</li>
</ul>

<p>Entonces la relación real sería:</p>
<ul>
  <li><strong>Caché</strong> → hecha con SRAM</li>
  <li><strong>RAM principal</strong> → hecha con DRAM</li>
</ul>

<h3> En resumen</h3>
<p>Las pirámides están bien en jerarquía funcional (muestran qué se usa antes o después).</p>
<p>Pero no significan que “la caché esté dentro de la RAM”. Más bien, ambas son tipos de memoria volátil, construidas con tecnologías distintas (<strong>SRAM</strong> y <strong>DRAM</strong>).</p>

    <img src="../../img/guia_274.png" alt="" width="600">

    <hr>
    <h3>⚔️ 4. Y en comparación con las otras arquitecturas del ejemplo que muestras:</h3>
...
<table>
  ...
  <tbody>
    <tr>
      <td>x86-64 / x64</td>
      <td>CISC</td>
      <td>PCs, portátiles, servidores (Intel, AMD)</td>
    </tr>
    <tr>
      <td>ARM64 (AArch64)</td>
      <td>RISC</td>
      <td>Móviles, tablets, Macs M1/M2/M3</td>
    </tr>
    <tr>
      <td>RISC-V (64-bits)</td>
      <td>RISC abierto</td>
      <td>Arquitectura moderna y libre</td>
    </tr>
  </tbody>
</table>

<p>✅ Así que sí:</p>
<p><strong>x64 = x86-64 = AMD64 = Intel 64</strong></p>
<p>Todas se refieren a la arquitectura de 64 bits compatible con los procesadores Intel/AMD de escritorio.</p>

<hr>

<h3>⚙️ 1. Qué significa “núcleo”</h3>
<p>Un <strong>núcleo (core)</strong> es una unidad de procesamiento independiente dentro del chip del procesador.
Cada núcleo tiene sus propios componentes básicos:</p>

<ul>
  <li>Unidad aritmético-lógica (ALU)</li>
  <li>Unidad de control</li>
  <li>Registros</li>
  <li>Caché L1 (y a veces L2)</li>
</ul>

<p>Así, cada núcleo puede ejecutar un hilo de ejecución (<em>thread</em>) de manera independiente.
Por eso un procesador de 8 núcleos puede ejecutar 8 tareas simultáneamente (o más si tiene <em>hyper-threading</em>).</p>
...
<h3>🧠 2. Cómo se organiza la caché</h3>
<p>Los procesadores modernos usan una jerarquía de cachés:</p>
...
<table>
  ...
  <tbody>
    <tr>
      <td>L1</td>
      <td>Muy pequeña (32–64 KB)</td>
      <td>Muy rápida</td>
      <td>Privada de cada núcleo</td>
    </tr>
    <tr>
      <td>L2</td>
      <td>Intermedia (256 KB–1 MB)</td>
      <td>Rápida</td>
      <td>Generalmente privada, a veces compartida por 2 núcleos</td>
    </tr>
    <tr>
      <td>L3</td>
      <td>Grande (varios MB)</td>
      <td>Más lenta</td>
      <td>Compartida entre todos los núcleos</td>
    </tr>
    <tr>
      <td>RAM (DRAM)</td>
      <td>Mucho más grande (GBs)</td>
      <td>Más lenta</td>
      <td>Compartida por todo el sistema</td>
    </tr>
  </tbody>
</table>
...
<ul>
  <li>Cada núcleo tiene su propia L1 (y normalmente su propia L2).</li>
  <li>Todos los núcleos comparten una sola L3, para intercambiar datos más rápido entre sí.</li>
</ul>

<h3>🧩 3. Ejemplo: tu CPU (Intel Core i5-12500H)</h3>
<p>Tu procesador es <strong>híbrido</strong> (arquitectura “Alder Lake”) y tiene:</p>

<ul>
  <li>4 núcleos de rendimiento (P-cores)</li>
  <li>8 núcleos de eficiencia (E-cores)</li>
</ul>

<p>Cada grupo de núcleos tiene su propia jerarquía de cachés:</p>
<ul>
  <li>L1 y L2 privadas por núcleo o por grupo.</li>
  <li>L3 compartida entre todos los núcleos.</li>
</ul>

<p>Así, el chip funciona como una red de núcleos que cooperan compartiendo una caché común de último nivel.</p>
...
<table>
  ...
  <tbody>
    <tr>
      <td>ALU / Registros</td>
      <td>✅ Sí</td>
      <td>Cada núcleo tiene los suyos.</td>
    </tr>
    <tr>
      <td>Caché L1</td>
      <td>✅ Sí</td>
      <td>Totalmente privada.</td>
    </tr>
    <tr>
      <td>Caché L2</td>
      <td>✅ Generalmente sí</td>
      <td>A veces compartida por 2 núcleos.</td>
    </tr>
    <tr>
      <td>Caché L3</td>
      <td>❌ No</td>
    ...
    <tr>
      <td>Memoria RAM</td>
      <td>❌ No</td>
      <td>Compartida por todo el procesador.</td>
    </tr>
  </tbody>
</table>

    <img src="../../img/guia_275.png" alt="" width="600">
