...
<section class="card">
    <h2>🧠 En conjunto</h2>
    <p>El fragmento equivale a la siguiente estructura en C++:</p>
    <pre><code>i = 0;
while (i <= 11) {
    // cuerpo del bucle en loc_140001529
}</code></pre>

    <p class="note">Si prefieres verlo como flujo de instrucciones:</p>

    <table>
      <thead>
        <tr><th>Paso</th><th>Instrucción</th><th>Efecto lógico</th><th>Comentario</th></tr>
      </thead>
      <tbody>
        <tr>
          <td>1</td>
          <td><code>mov [rbp+i], 0</code></td>
          <td><code>i = 0</code></td>
          <td>Inicializa el contador</td>
        </tr>
        <tr>
          <td>2</td>
          <td><code>jmp loc_140001618</code></td>
          <td>Salta al inicio del bucle</td>
          <td>Evita ejecutar el cuerpo antes de verificar</td>
        </tr>
        <tr>
          <td>3</td>
          <td><code>cmp [rbp+i], 0Bh</code></td>
          <td>Compara <code>i</code> con 11</td>
          <td>Evalúa la condición</td>
        </tr>
        <tr>
          <td>4</td>
          <td><code>jbe loc_140001529</code></td>
          <td>Si <code>i ≤ 11</code>, entra al cuerpo</td>
          <td>Controla la iteración</td>
        </tr>
      </tbody>
    </table>
  </section>
  ...
  <section class="card">
    <h2>🧩 En contexto general</h2>
    <p class="muted">Uniendo esto con lo que ya habías visto (la comprobación de longitud > 11), la estructura completa suele ser:</p>
    <pre><code>if (len > 0x0B) {      // si la longitud del input es mayor a 11
    i = 0;
    while (i <= 0x0B) {    // recorre los primeros 12 caracteres
        // ... lógica en loc_140001529 (probablemente comparaciones por carácter)
        i++;
    }
}</code></pre>
    <p class="note">Normalmente en <code>loc_140001529</code> verás comparaciones con <code>input[i]</code> y constantes (caracteres o valores codificados) que realizan la verificación del password.</p>
  </section>
    ...
    <hr>
    <img src="../../../img/guia_255.png" alt="">
 
    <h1>Comparación y salto: <code>cmp [rbp+i], 3</code> / <code>ja</code></h1>

    <h2>Código involucrado</h2>
    <pre><code>cmp     [rbp+i], 3
ja      short loc_14000158A</code></pre>

    <h2>Qué significa</h2>
    <p class="note"><code>cmp [rbp+i], 3</code> → compara el valor de <code>i</code> con <code>3</code>.</p>
    <p class="note">Internamente realiza <code>i - 3</code> (sin modificar <code>i</code>).</p>
    <p class="note"><code>ja loc_14000158A</code> → <strong>Jump if Above</strong> → salta si <code>i &gt; 3</code>.</p>
    <p class="muted">“Above” significa “mayor que” en comparación <em>sin signo</em>.</p>

    <h2>Caso actual: <code>i = 0</code></h2>
    <p>👉 Evaluamos la condición:</p>
    <div class="step">
      <p><strong>i = 0</strong></p>
      <p>¿0 &gt; 3? → <strong>Falso</strong></p>
    </div>

    <p class="note">Por lo tanto, <strong>NO</strong> se cumple la condición del salto <code>ja</code>. El flujo no va hacia <code>loc_14000158A</code> (a la derecha o abajo), sino que sigue directamente hacia la siguiente instrucción secuencial —en el diagrama esto suele aparecer a la izquierda o abajo del bloque actual (dependiendo de cómo IDA dibuje el “fallthrough”).</p>

    <h2>Visualmente en el diagrama</h2>
    <p class="note">Cuando el salto se cumple (<code>ja</code>), IDA lo dibuja con una <strong>línea verde</strong> → eso sería cuando <code>i &gt; 3</code>.</p>
    <p class="note">Cuando no se cumple (fallthrough), IDA lo dibuja con una <strong>línea roja o azul</strong> → eso ocurre cuando <code>i ≤ 3</code>.</p>

    <p class="muted">En tu imagen, al inicio (<code>i = 0</code>):</p>
    <ul>
      <li>No se cumple <code>i &gt; 3</code>.</li>
      <li>Entonces el flujo sigue por la línea de “no salto” (la izquierda o abajo).</li>
      <li class="ok">Por eso dices correctamente: “este va a la izquierda en el primer <code>i = 0</code>” ✅</li>
    </ul>

    <h2>En resumen — paso a paso (primer ciclo)</h2>
    <ol>
      <li><strong>i = 0</strong></li>
      <li><code>cmp [rbp+i], 3</code></li>
      <li><code>ja ...</code> → ¿0 &gt; 3? → <strong>No</strong></li>
      <li>No salta, sigue la ejecución normal (a la izquierda)</li>
      <li>Ejecuta el resto del cuerpo del <code>while</code></li>
    </ol>

    <hr>
    <img src="../../../img/guia_256.png" alt="" width="700">

     <h1>🔹 Flujo de comparaciones en el ciclo</h1>

    <p>¡Exacto! Cuando <code>i &gt; 3</code> (es decir, <code>i = 4, 5, 6, 7, 8, 9, 10, 11</code>):</p>

    <pre><code>loc_140001529:
...
cmp     [rbp+i], 3
ja      short loc_14000158A    ; si i &gt; 3, SALTA aquí
    </code></pre>

    <p>Entonces va a:</p>

    <pre><code>loc_14000158A:
cmp     [rbp+i], 7             ; compara i con 7
ja      short loc_140001503    ; si i &gt; 7, salta a loc_140001503
    </code></pre>

    <img src="../../../img/guia_257.png" alt="" width="700">
    <h3>RAMA IZQUIERDA (i = 0-3, primeros 4 caracteres):</h3>
<pre><code>mov     edx, 0
lea     rax, _ZL10claves_xor
mov     rcx, rax          ; this
call    _ZNKSt5arrayIhLy3EEixEy  ; std::array&lt;uchar,3u&gt;::operator[](ulong long)
...
lea     rax, _ZL5clave
mov     rcx, rax          ; this
call    _ZNKSt5arrayIhLl12EEixEy  ; std::array&lt;uchar,12u&gt;::operator[](ulong long)
movzx   eax, byte ptr [rax]
cmp     [rbp+tmp], al
setz    al
test    al, al
...
<p><strong>Validación para primeros 4 caracteres:</strong></p>
<ul>
  <li>Accede a <code>claves_xor[0]</code></li>
  <li>Compara con <code>clave[i]</code> usando el carácter ingresado</li>
  <li>Si no coincide → va a <code>loc_140001613</code> (error)</li>
</ul>
...
<h3>RAMA CENTRAL (i = 4-7):</h3>
<pre><code>loc_14000158A:
cmp     [rbp+i], 7
ja      short loc_140001503
...
lea     rax, _ZL5clave
mov     rcx, rax          ; this
call    _ZNKSt5arrayIhLl12EEixEy
movzx   eax, byte ptr [rax]
cmp     [rbp+tmp], al
setnz   al
test    al, al
jz      short loc_140001613
</code></pre>

<p><strong>Validación para caracteres 5-8 (i=4-7):</strong></p>
<ul>
  <li>Primero verifica si <code>i &gt; 7</code> → si sí, va a <code>loc_140001503</code></li>
  <li>Si <code>i ≤ 7</code>: usa <code>claves_xor[1]</code></li>
  <li>Hace XOR con el carácter</li>
  <li>Compara con <code>clave[i]</code></li>
</ul>

<h3>RAMA DERECHA (i = 8-11, últimos 4 caracteres):</h3>
<pre><code>loc_140001503:
mov     edx, 2
lea     rax, _ZL10claves_xor
mov     rcx, rax          ; this
call    _ZNKSt5arrayIhLy3EEixEy
...
setnz   al
test    al, al
jnz     short loc_140001628
</code></pre>

<p><strong>Validación para caracteres 9-12 (i=8-11):</strong></p>
<ul>
  <li>Usa <code>claves_xor[2]</code></li>
  <li>Hace XOR con el carácter</li>
  <li>Compara con <code>clave[i]</code></li>
</ul>

<h3>RESUMEN:</h3>
<p>El password se valida en 3 secciones:</p>
<ul>
  <li><strong>Posiciones 0-3:</strong> Sin XOR, comparación directa</li>
  <li><strong>Posiciones 4-7:</strong> XOR con <code>claves_xor[1]</code></li>
  <li><strong>Posiciones 8-11:</strong> XOR con <code>claves_xor[2]</code></li>
</ul>

    <hr>
    <strong>La linea verde en el primer y segundo bloque conduce a una caja que incremente el index, y redirige a la
         parte superior</strong>
         <br><br>
    <img src="../../../img/guia_258.png" alt="" width="390">
    <img src="../../../img/guia_259.png" alt="" width="390">

    <p>Pequeña caja a la que conduce la linea verde de los bloques 1 y 2</p>
    <pre>  <code>loc_140001613:
add     [rbp+i], 1</code></pre> 

    <img src="../../../img/guia_260.png" alt="" width="700">

    <p>La diferencia de desicion esta en la linea roja para bloques 1 y 2</p>

    <h3><code>jmp fail</code> (salto normal/largo):</h3>
<ul>
  <li>Usa un desplazamiento de 32 bits</li>
  <li>Puede saltar a cualquier lugar del programa (hasta ±2GB)</li>
  <li>Ocupa 5 bytes en el código máquina</li>
  <li>Se usa cuando el destino está lejos</li>
</ul>

<h3><code>jmp short fail</code> (salto corto):</h3>
<ul>
  <li>Usa un desplazamiento de 8 bits</li>
  <li>Solo puede saltar ±128 bytes desde la posición actual</li>
  <li>Ocupa 2 bytes en el código máquina (más eficiente)</li>
  <li>Se usa cuando el destino está cerca</li>
</ul>
    <hr>

    <h3>Bloques 1 y 2:</h3>
<pre><code>test    al, al
jz      loc_140001613    ; si ES IGUAL (zero flag), continúa ✓
; si NO es igual, cae a fail ❌
</code></pre>

<h3>Bloque 3:</h3>
<pre><code>setnz   al               ; al = 1 si NO son iguales
test    al, al
jnz     short loc_140001628   ; si NO ES IGUAL, salta ❌
; si SON iguales, cae a loc_140001613 ✓
</code></pre>

<h3>¿Por qué la diferencia?</h3>
<p>Mira las instrucciones previas:</p>

<h4>Bloques 1 y 2:</h4>
<pre><code>cmp     [rbp+tmp], al
setz    al               ; al = 1 si SON iguales
test    al, al
jz      fail             ; si al = 0 (NO iguales) → fail
</code></pre>

<h4>Bloque 3:</h4>
<pre><code>cmp     [rbp+tmp], al
setnz   al               ; al = 1 si NO son iguales (inverso!)
test    al, al
jnz     loc_140001628    ; si al = 1 (NO iguales) → error
</code></pre>

<h3>Entonces:</h3>
<ul>
  <li><strong>Bloques 1 y 2:</strong> usan <code>setz</code> → saltan si son iguales ✓</li>
  <li><strong>Bloque 3:</strong> usa <code>setnz</code> → salta si NO son iguales ❌ (lógica invertida)</li>
</ul>

<p>Ambos hacen lo mismo funcionalmente, solo que el bloque 3 tiene la lógica al revés. Probablemente una optimización del compilador.</p>


<h3>🔹 Qué significa "[rbp+ch_0]"</h3>
<p>Cuando ves corchetes <strong>[...]</strong> en ensamblador, eso no significa el valor literal — significa “el contenido que hay en la dirección de memoria”.</p>
<p><strong>rbp</strong> es el Base Pointer, o puntero base del stack frame (la pila de la función).</p>
<p><strong>ch_0</strong> es una variable local que el compilador reservó en la pila, en este caso (según tu código):</p>
<p><strong>ch_0 = byte ptr -11h</strong></p>
<p>o sea, está 11h (17) bytes debajo del valor actual de <strong>rbp</strong>.</p>
<p>Entonces:</p>
<p><strong>[rbp+ch_0] ≡ [rbp - 0x11] ≡ el byte guardado en la variable local ch_0</strong></p>
