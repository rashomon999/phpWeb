...
<div class="seccion izquierda"> 
     ...
     <h1>1. Clasificación Principal</h1>
    
    <h2>A. Formatos Nativos</h2>
    <p><em>(Procesados directamente por la CPU)</em></p>
    
    <div class="native">
        <h3>1. Enteros (Integers)</h3>
        <h4>Con signo</h4>
        <p>\(\text{int8\_t}\), \(\text{int16\_t}\), \(\text{int32\_t}\), \(\text{int64\_t}\) (complemento a 2).</p>
        <div class="example">
            Ejemplo: \(\text{int32\_t}\ x = -500\); → Rango: \(-2^{31}\) a \(2^{31}-1\).
        </div>
        
        <h4>Sin signo</h4>
        <p>\(\text{uint8\_t}\), \(\text{uint16\_t}\), etc.</p>
        <div class="example">
            Ejemplo: \(\text{uint8\_t}\ y = 255\); → Rango: 0 a 255.
        </div>
        
        <p class="usage">Usos: Contadores, índices, direcciones de memoria.</p>
    </div>
    
    <div class="native">
        <h3>2. Punto Flotante (IEEE 754)</h3>
        <h4>Precisión simple (float)</h4>
        <p>32 bits (1 signo + 8 exponente + 23 mantisa).</p>
        <div class="example">
            Ejemplo: \(3.14f\)
        </div>
        
        <h4>Precisión doble (double)</h4>
        <p>64 bits (1 + 11 + 52).</p>
        <div class="example">
            Ejemplo: \(3.141592653589793\)
        </div>
        
        <p class="usage">Usos: Cálculos científicos, gráficos 3D.</p>
    </div>
    
    <div class="native">
        <h3>3. Caracteres (char) y Booleanos (bool)</h3>
        <p>\(\text{char}\): 8 bits (ASCII) o 16/32 bits (Unicode).</p>
        <p>\(\text{bool}\): 1 byte (\(\text{true}=1\), \(\text{false}=0\)).</p>
    </div>
    
    <h2>B. Formatos No Nativos</h2>
    <p><em>(Requieren librerías o manejo software)</em></p>
    
    <div class="non-native">
        <h3>1. Punto Fijo (Fixed-Point)</h3>
        <p>Entero + fracción con escala fija (ej.: Q7.8 = 7 bits enteros + 8 fraccionarios).</p>
        <div class="example">
            Ejemplo: 1.75 se almacena como \(\text{0x01C0}\) (escala 128).
        </div>
        <p class="usage">Usos: DSP, sistemas embebidos sin FPU.</p>
    </div>
    
    <div class="non-native">
        <h3>2. Decimales de Precisión Exacta</h3>
        <p>\(\text{decimal}\) (C#/Python): Para evitar errores de redondeo en finanzas.</p>
        <div class="example">
            Ejemplo: \(\text{Decimal.Parse}("123.45")\)
        </div>
    </div>
    
    <div class="non-native">
        <h3>3. Formatos de Alta Precisión</h3>
        <p>\(\text{BigInteger}\) (Java/C#): Enteros de tamaño arbitrario.</p>
        <p>\(\text{BigFloat}\) (librerías MPFR): Punto flotante con precisión configurable.</p>
    </div>
    
    <div class="non-native">
        <h3>4. BCD (Binary-Coded Decimal)</h3>
        <p>Cada dígito decimal ocupa 4 bits (ej.: 12 = \(0001\ 0010\)).</p>
        <p class="usage">Usos: Sistemas legacy, calculadoras.</p>
    </div>
    
    <h2>2. Comparativa Rápida</h2>
    <table>
        <tr>
            <th>Tipo</th>
            <th>Ejemplo</th>
        ...
        <tr>
            <td><strong>Entero (nativo)</strong></td>
            <td>\(\text{int32_t}\ x = -42;\)</td>
            <td>Máxima velocidad, precisión exacta</td>
            <td>Rango limitado</td>
            <td>Contadores, índices</td>
        </tr>
        <tr>
            <td><strong>Float (nativo)</strong></td>
            <td>\(\text{float}\ \pi = 3.14f;\)</td>
            <td>Rango amplio</td>
            <td>Error de redondeo</td>
            <td>Gráficos 3D, física</td>
        </tr>
        <tr>
            <td><strong>Punto Fijo</strong></td>
            <td>\(\text{Q7.8}:\ 1.75 \rightarrow \text{0x01C0}\)</td>
            <td>Precisión controlada, sin FPU</td>
            <td>Manejo manual del punto</td>
            <td>DSP, sistemas embebidos</td>
        </tr>
        <tr>
            <td><strong>Decimal</strong></td>
            <td>\(\text{Decimal.Parse}("10.99")\)</td>
            <td>Precisión exacta en decimales</td>
            <td>Más lento que nativos</td>
            <td>Finanzas, contabilidad</td>
        </tr>
        <tr>
            <td><strong>BigInteger</strong></td>
            <td>\(\text{BigInteger.Parse}("999...")\)</td>
            <td>Tamaño arbitrario</td>
...
<div class="seccion derecha">
 ...
 <h3>📌 Formatos numéricos en computación</h3>


<h4>🔹 Aspectos clave</h4>
<ul>
<li><strong>Cantidad de bits</strong> → determina cuánto espacio ocupa en memoria.</li>
<li><strong>Rango</strong> → intervalo de valores posibles.</li>
<li><strong>Precisión</strong> → qué tan exacta es la representación (especialmente en flotantes).</li>
</ul>


<h4>🔸 Enteros (int)</h4>
<p>Representados en binario puro.</p>
<ul>
<li><strong>Con signo (signed):</strong> un bit se reserva para el signo. Ejemplo 8 bits: –128 a 127.</li>
<li><strong>Sin signo (unsigned):</strong> todos los bits son magnitud. Ejemplo 8 bits: 0 a 255.</li>
</ul>


<p><strong>✅ Características:</strong></p>
<ul>
<li>Rango depende directamente de la cantidad de bits.</li>
<li>Precisión: exacta dentro del rango (no hay error decimal).</li>
</ul>


<p><strong>📊 Ejemplos comunes:</strong></p>
<ul>
<li>int8 → –128 a 127</li>
<li>uint8 → 0 a 255</li>
<li>int32 → –2,147,483,648 a 2,147,483,647</li>
<li>int64 → ~±9.22e18</li>
</ul>


<h4>🔸 Flotantes (float, double)</h4>
<p>Usan el estándar IEEE 754 (notación científica binaria).</p>
<p><strong>Estructura:</strong> signo + exponente + mantisa</p>


<ul>
<li><strong>Exponente:</strong> rango (qué tan grandes o pequeños los números).</li>
<li><strong>Mantisa:</strong> precisión (cuántas cifras significativas).</li>
</ul>


<p><strong>📊 Ejemplos comunes:</strong></p>
<ul>
<li><strong>float (32 bits):</strong> 1 signo, 8 exponente, 23 mantisa → ~7 cifras decimales. Rango: ±10³⁸</li>
<li><strong>double (64 bits):</strong> 1 signo, 11 exponente, 52 mantisa → ~15 cifras decimales. Rango: ±10³⁰⁸</li>
</ul>


<h4>🔹 Relación entre rango, precisión y bits</h4>
<ul>
<li><strong>En enteros:</strong> más bits = más rango. Precisión siempre absoluta dentro del rango.</li>
<li><strong>En flotantes:</strong> más bits permiten asignar más tanto al exponente como a la mantisa.</li>
<li>Más exponente → más rango, menos precisión.</li>
<li>Más mantisa → más precisión, menos rango.</li>
<li>Ejemplo real: <em>double</em> (64 bits) tiene más rango y más precisión que <em>float</em> (32 bits), porque tiene más bits totales.</li>
</ul>


<h4>✅ Resumen final</h4>
<ul>
<li><strong>int:</strong> números enteros exactos, rango fijo según los bits.</li>
<li><strong>float:</strong> números reales aproximados, 32 bits, ~7 cifras decimales, rango ±10³⁸.</li>
<li><strong>double:</strong> números reales aproximados, 64 bits, ~15 cifras decimales, rango ±10³⁰⁸.</li>
<li><strong>Bits:</strong> determinan tanto rango como precisión.</li>
<li>En flotantes, se equilibra rango vs precisión según cómo se reparten los bits entre exponente y mantisa.</li>
</ul>
 


<h3>Regla general para enteros con signo (complemento a dos)</h3>
<p>
<strong>−2<sup>n−1</sup> ≤ A<sub>int</sub> ≤ 2<sup>n−1</sup> − 1</strong>  
</p>
<p>donde <strong>n</strong> es la cantidad de bits.</p>

<h4>Ejemplos</h4>
<ul>
  <li><strong>8 bits (signed int8):</strong> −2<sup>7</sup> = −128 ≤ A ≤ 2<sup>7</sup> − 1 = 127</li>
  <li><strong>16 bits (signed int16):</strong> −2<sup>15</sup> = −32768 ≤ A ≤ 32767</li>
  <li><strong>32 bits (signed int32):</strong> −2<sup>31</sup> = −2,147,483,648 ≤ A ≤ 2,147,483,647</li>
</ul>

<h3>Regla general para enteros sin signo (unsigned)</h3>
<p>
<strong>0 ≤ A<sub>uint</sub> ≤ 2<sup>n</sup> − 1</strong>
</p>
<p>Ejemplo con 8 bits → 0 ≤ A ≤ 255</p>

<h3>Máximos y mínimos en complemento a dos</h3>
<p>En complemento a dos (signed int) con <strong>n</strong> bits:</p>
<ul>
  <li><strong>Máximo valor positivo:</strong> 0111…111 = 2<sup>n−1</sup> − 1</li>
  <li><strong>Mínimo valor negativo:</strong> 1000…000 = −2<sup>n−1</sup></li>
</ul>
