...
<h4>Ejemplo con 8 bits</h4>
<ul>
  <li><strong>Máximo positivo:</strong> 0111 1111 = 127 (esto es 2<sup>7</sup> − 1)</li>
  <li><strong>Mínimo negativo:</strong> 1000 0000 = −128 (esto es −2<sup>7</sup>)</li>
</ul>

<h3>Comportamiento al sumar</h3>
<p>
Si estás en <strong>0111 1111 (127)</strong> y le sumas 1, obtienes <strong>1000 0000</strong>,  
que en complemento a dos corresponde a <strong>−128</strong>, es decir, el mínimo valor representable.
</p>

 

 <h3>El rango no es simétrico</h3>
<p>
En <strong>n</strong> bits: −2<sup>n−1</sup> ≤ A ≤ 2<sup>n−1</sup> − 1  
</p>
<ul>
  <li>Mínimo negativo: −2<sup>n−1</sup></li>
  <li>Máximo positivo: +(2<sup>n−1</sup> − 1)</li>
</ul>

<h4>Ejemplo con 8 bits</h4>
<ul>
  <li>Negativos: desde −128 hasta −1 (128 valores)</li>
  <li>Cero: 0 (1 valor)</li>
  <li>Positivos: desde 1 hasta 127 (127 valores)</li>
</ul>
<p>Total: 128 + 1 + 127 = 256 = 2<sup>8</sup></p>



<h3>Complemento a dos y la representación del cero</h3>

<p>En <strong>complemento a dos</strong>, el cero tiene una sola representación:</p>

<ul>
  <li>Con todos los bits en <strong>0</strong>.</li>
</ul>

<p><strong>Ejemplo con 8 bits:</strong></p>
<p>0000 0000 = 0</p>

<h4>Diferencia con otros sistemas de representación</h4>

<p>En <strong>signo y magnitud (sign-magnitude)</strong> había +0 y –0 porque se usaba un bit para el signo y el resto para la magnitud:</p>
...
<p>En complemento a dos, se evita esa ambigüedad porque el patrón <strong>1000...000</strong> (el más negativo) ya está ocupado para representar <strong>-2^(n-1)</strong>.</p>

<p>→ Por eso el cero queda único: <strong>0000...000</strong>.</p>

<h4>Resumen</h4>

<ul>
  <li>En complemento a dos, <strong>0 = 000...000</strong> (única representación).</li>
  <li>Eso es parte de la razón por la cual el rango no es simétrico: hay un valor negativo extra para aprovechar el bit de signo.</li>
</ul>

    <hr>

     <h1>Conversión de Hexadecimal IEEE-754 a Decimal</h1>
    
    <h2>1. Número Hexadecimal</h2>
    <p>Tenemos el siguiente número hexadecimal que representa un valor en formato IEEE-754 de 32 bits:</p>
    <p><strong>40358000<sub>16</sub></strong></p>
    
    <h2>2. Conversión a Binario</h2>
    <p>Cada dígito hexadecimal corresponde a 4 bits:</p>
    <ul>
        <li>4<sub>16</sub> = 0100<sub>2</sub></li>
        <li>0<sub>16</sub> = 0000<sub>2</sub></li>
        <li>3<sub>16</sub> = 0011<sub>2</sub></li>
        <li>5<sub>16</sub> = 0101<sub>2</sub></li>
        <li>8<sub>16</sub> = 1000<sub>2</sub></li>
        <li>0<sub>16</sub> = 0000<sub>2</sub></li>
        <li>0<sub>16</sub> = 0000<sub>2</sub></li>
        <li>0<sub>16</sub> = 0000<sub>2</sub></li>
    </ul>
    <p>Uniendo todos los bits:</p>
    <p><strong>0100 0000 0011 0101 1000 0000 0000 0000</strong></p>
    <p>O en formato continuo:</p>
    <p><strong>01000000001101011000000000000000</strong></p>
    
    <h2>3. División según formato IEEE-754 (32 bits)</h2>
    <p>El formato de precisión simple (32 bits) tiene:</p>
    <ul>
        <li>1 bit de signo</li>
        <li>8 bits de exponente</li>
        <li>23 bits de fracción (mantisa)</li>
    </ul>
    <p>Separando nuestro número:</p>
    <p><strong>0 | 10000000 | 01101011000000000000000</strong></p>
    <p>↑ &nbsp; &nbsp; &nbsp; &nbsp; ↑ &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; ↑</p>
    <p>signo &nbsp; exponente (E) &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; mantisa</p>
    
    <h2>4. Identificar E (exponente)</h2>
    <p>La parte que corresponde al exponente son los 8 bits después del bit de signo:</p>
    <p><strong>10000000<sub>2</sub></strong></p>
    <p>Convertido a decimal:</p>
    <p>10000000<sub>2</sub> = 128<sub>10</sub></p>
    
    <h2>5. Calcular el exponente real (e)</h2>
    <p>El estándar IEEE 754 utiliza un sesgo (bias) de 127 para el exponente:</p>
    <p>e = E - 127 = 128 - 127 = 1</p>
    
    <h2>6. Interpretar la mantisa</h2>
    <p>La mantisa es: <strong>01101011000000000000000</strong></p>
    <p>En el formato IEEE-754, se añade un 1 implícito al principio:</p>
    <p>Mantisa completa: <strong>1.01101011000000000000000</strong></p>
    
    <h2>7. Calcular el valor decimal</h2>
    <p>La fórmula para el valor es: (-1)<sup>signo</sup> × (1 + mantisa) × 2<sup>exponente</sup></p>
    <p>Para nuestro número:</p>
    <ul>
        <li>Signo = 0 (positivo)</li>
        <li>Mantisa = 1 + 0.01101011000000000000000<sub>2</sub> = 1 + 0.4185791015625<sub>10</sub> = 1.4185791015625<sub>10</sub></li>
        <li>Exponente = 1</li>
    </ul>
    <p>Resultado = 1 × 1.4185791015625 × 2<sup>1</sup> = 2.837158203125</p>
    
    <h2>8. Resultado final</h2>
    <p>El número hexadecimal <strong>40358000<sub>16</sub></strong> en formato IEEE-754 representa el valor decimal:</p>
    <p><strong>2.837158203125</strong></p>
...
<div class="seccion derecha">
     ...
     <h1>Cálculo del Exponente en Formato IEEE-754</h1>
    
    <h2>Para el número hexadecimal: 40FD8000₁₆</h2>
    
    <h3>Paso 1: Conversión a binario (32 bits)</h3>
    <p>40FD8000₁₆ = 0100 0000 1111 1101 1000 0000 0000 0000₂</p>
    
    <h3>Paso 2: División según formato IEEE-754 (1|8|23)</h3>
    <table border="1" cellpadding="5">
        <tr>
            <th>signo (1 bit)</th>
            <th>exponente (8 bits)</th>
            <th>fracción/mantisa (23 bits)</th>
        </tr>
        <tr>
            <td align="center">0</td>
            <td align="center">10000001</td>
            <td align="center">11111011000000000000000</td>
        </tr>
    </table>
    
    <h3>Paso 3: Cálculo del exponente almacenado (E)</h3>
    <p>Exponente almacenado (E) = 10000001₂</p>
    <p>Conversión a decimal: 10000001₂ = 129₁₀</p>
    
    <h3>Paso 4: Cálculo del exponente real (e)</h3>
    <p>El estándar IEEE-754 utiliza un sesgo (bias) de 127 para exponentes de 8 bits:</p>
    <p>Exponente real (e) = E - 127 = 129 - 127 = 2</p>
    
    <h3>Paso 5: Interpretación del resultado</h3>
    <p>Con un exponente real e = 2, el número se calcula como:</p>
    <p>(+1) × 1.11111011₂ × 2²</p>
    <p>Donde:</p>
    <ul>
        <li>El bit de signo es 0, por lo que el signo es positivo (+1)</li>
        <li>Se añade el 1 implícito a la mantisa: 1.11111011₂</li>
        <li>El valor se escala por 2² según el exponente calculado</li>
    </ul>
    
    <h3>Resumen del proceso</h3>
    <ol>
        <li>Convertir el número hexadecimal a binario de 32 bits</li>
        <li>Separar según el formato IEEE-754: signo (1 bit), exponente (8 bits), mantisa (23 bits)</li>
        <li>Convertir el exponente almacenado (E) de binario a decimal</li>
        <li>Calcular el exponente real: e = E - 127</li>
        <li>Utilizar el exponente real para calcular el valor final del número</li>
    </ol>

    <p>o tambien podemos ver consultar en la pagina:</p>
    <img src="../../img/guia_173.png" alt="" width="600">

    <hr>

    <h1>📉 ¿Se pierde un bit?</h1>
    
    <p>Sí, al igualar exponentes se desplaza a la derecha la mantisa del número menor.
    Eso implica que los bits que "se corren fuera" se pierden (o mejor dicho, se van a los guard bits: guard, round, sticky).</p>
    
    <h2>Ejemplo simple en binario:</h2>
    
    <p>1.0000000<sub>2</sub> × 2<sup>3</sup> + 1.0000000<sub>2</sub> × 2<sup>0</sup></p>
    
    <ul>
        <li>Primer número: 8</li>
        <li>Segundo número: 1</li>
    </ul>
    
    <p>Para sumarlos, hay que igualar exponentes (subir el de 2<sup>0</sup> hasta 2<sup>3</sup>):</p>
    
    <p>1.0000000<sub>2</sub> × 2<sup>3</sup> + 0.0010000<sub>2</sub> × 2<sup>3</sup></p>
    
    <p>Ese 0.0010000<sub>2</sub> viene de desplazar la mantisa del "1" tres lugares a la derecha.
    👉 Aquí se perdieron bits en la parte baja de la mantisa.</p>
    
    <h2>📌 Respuesta corta</h2>
    
    <p>Sí, siempre que se igualan exponentes se pierde precisión en el número más pequeño, porque su mantisa se desplaza 
    a la derecha.</p>
    
    <h2>Explicación adicional:</h2>
    
    <p>En las operaciones con números en punto flotante, cuando los exponentes son diferentes, es necesario ajustarlos 
        al mismo valor para poder realizar la operación. Esto implica desplazar la mantisa del número con el exponente 
        más pequeño hacia la derecha, lo que puede causar la pérdida de bits de menor significancia.</p>
    
    <p>Estos bits "perdidos" no desaparecen completamente, sino que se almacenan temporalmente en los 
        <strong>bits de guarda</strong> (guard bits), que se utilizan para mejorar la precisión durante el redondeo, 
        pero aún así puede producirse una pérdida de precisión en el resultado final.</p>


    <hr>
     <h1>📌 Por qué decimos que es "el menos significativo"</h1>
    
    <div class="explanation">
        <p>En un número binario normalizado tipo:</p>
        <div class="binary-number">1.00100011100000000000000</div>
        
        <p>El último <span class="highlight">1</span> que aparece es el último bit de información real.</p>
        
        <p>Todo lo que viene después (ceros) ya no aporta valor → por eso, en la práctica, ese último <span class="highlight">1</span> marca la frontera de lo significativo.</p>
    </div>
    
    <div class="example">
        <p>👉 Entonces, aunque en teoría el LSB "fijo" es siempre la posición 23 de la mantisa, en la práctica el último 1 que encuentres es el bit menos significativo real de ese número porque a partir de ahí ya no hay nada más que perder (solo ceros).</p>
        
        <p>Entonces: miramos el número normalizado y miramos cuántos <span class="zero-highlight">ceros</span> después del último <span class="highlight">1</span> hay, esos son los lugares que puedo perder o la cantidad de 0 a la derecha del bit menos significativo.</p>
    </div>
    
    <h2>Ejemplo práctico:</h2>
    
    <p>Consideremos el número binario normalizado:</p>
    <div class="binary-number">1.0101<span class="highlight">1</span><span class="zero-highlight">0000000000000</span></div>
    
    <ul>
        <li>El último <span class="highlight">1</span> está en la posición 5 después del punto</li>
        <li>Hay <span class="zero-highlight">12 ceros</span> después de este último 1</li>
        <li>Esto significa que podemos perder hasta 12 bits sin afectar la precisión real del número</li>
    </ul>
    
    <h2>Implicaciones en operaciones de punto flotante:</h2>
    
    <p>Cuando igualamos exponentes en operaciones aritméticas:</p>
    <ol>
        <li>Identificamos el último bit significativo (el último 1) en cada número</li>
        <li>Calculamos cuántos ceros hay después de este bit en cada número</li>
        <li>Al desplazar la mantisa, sabemos exactamente cuántos bits podemos perder sin afectar la precisión esencial</li>
        <li>Los bits que se pierden más allá del último 1 significativo no afectan el valor del número</li>
    </ol>
    
    <div class="conclusion">
        <p><strong>Conclusión:</strong> El verdadero "bit menos significativo" no es una posición fija en la mantisa, sino el último bit con valor 1 en la representación binaria del número. Todos los ceros que le siguen son prescindibles sin pérdida de información relevante.</p>
    </div>
