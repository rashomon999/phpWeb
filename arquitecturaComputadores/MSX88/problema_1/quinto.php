...
<div class="seccion izquierda"> 
    ...
    <section>
        <h2>C) jz - Jump if Zero</h2>
        <pre><code>cmp     [rbp+tmp], al        ; Comparar tmp con clave[i]
setnz   al                   ; al = (tmp != clave[i])
test    al, al               ; Testear al
jz      loc_140001613        ; Saltar si al == 0 (éxito)</code></pre>
        <p><strong>Condición:</strong> Salta si ZF=1 (resultado es cero)</p>
        <p><strong>Flags que revisa:</strong> ZF=1</p>
        <p><strong>Uso en el código:</strong></p>
        <ul>
            <li>Si tmp == clave[i] → al=0 → ZF=1 → jz SALTA → Éxito</li>
            <li>Si tmp != clave[i] → al=1 → ZF=0 → NO salta → Probar otra clave</li>
        </ul>
        <div class="example">
            <p>Ejemplo:</p>
            <p>tmp = 0x6E, clave[i] = 0x6E → Iguales → al=0 → jz SALTA</p>
            <p>tmp = 0x6E, clave[i] = 0x50 → Diferentes → al=1 → NO salta</p>
        </div>
    </section>

    <section>
        <h2>D) jnz - Jump if Not Zero</h2>
        <pre><code>cmp     [rbp+tmp], al        ; Comparar tmp con clave[i]
setnz   al                   ; al = (tmp != clave[i])
test    al, al               
jnz     short loc_140001628  ; Saltar si al != 0 (fallo total)</code></pre>
        <p><strong>Condición:</strong> Salta si ZF=0 (resultado NO es cero)</p>
        <p><strong>Flags que revisa:</strong> ZF=0</p>
        <p><strong>Uso en el código:</strong></p>
        <ul>
            <li>Si tmp != clave[i] → al=1 → ZF=0 → jnz SALTA → Fallo total</li>
            <li>Si tmp == clave[i] → al=0 → ZF=1 → NO salta → Continuar</li>
        </ul>
        <div class="example">
            <p>Ejemplo:</p>
            <p>tmp = 0x6E, clave[i] = 0x50 → Diferentes → al=1 → jnz SALTA → Fallo</p>
            <p>tmp = 0x6E, clave[i] = 0x6E → Iguales → al=0 → NO salta</p>
        </div>
    </section>
    
    <hr>
      <header>
      <h1 id="title">El símbolo <code>^</code> — ¿significa XOR?</h1>
      <p class="lead">Sí, : <code>^</code> normalmente representa <strong>XOR</strong> (operador exclusivo OR) en muchos lenguajes.</p>
    </header>

    <section>
      <p><strong>Lenguajes donde se usa así:</strong></p>
      <ul>
        <li><strong>C / C++ / Java / JavaScript / Python / etc.</strong></li>
      </ul>

      <p><strong>Ejemplo (Python):</strong></p>
      <pre class="code" aria-label="Ejemplo de XOR en Python"><code> a = 5  # 0101
 b = 3  # 0011

 c = a ^ b  # XOR bit a bit
 print(c)   # 6 (0110)</code></pre>

      <p>En este ejemplo la tabla verdad bit a bit es:</p>
      <div class="truth">
        0 ^ 0 = 0<br>
        0 ^ 1 = 1<br>
        1 ^ 0 = 1<br>
      ...
      <p>Así que sí: <code>^</code> = <strong>XOR bit a bit</strong> (exclusivo OR).</p>

      <div class="note">
        <strong>Nota:</strong> en lógica booleana pura (no bit a bit), también puedes usar <code>!=</code> o <code>a != b</code> como equivalente lógico de XOR cuando trabajas con valores booleanos, porque devuelve <em>true</em> solo cuando los valores son distintos.
      </div>
    </section>
...
<h2>🔍 Qué hace cada línea</h2>

<h3>1️⃣ <code>mov rbx, 1000h</code></h3>
<p>Carga el valor <code>0x1000</code> (4096 decimal) en el registro <code>rbx</code>.</p>
<p>⚠️ Esto no es una dirección válida de memoria del proceso, simplemente es un número.</p>

<h3>2️⃣ <code>lea rax, [rbx + 20h]</code></h3>
<p><strong>“Load Effective Address”</strong> → carga en <code>rax</code> la dirección calculada, pero no accede memoria.</p>
<p>Aquí solo hace:</p>
<pre><code>rax = rbx + 0x20  →  rax = 0x1000 + 0x20 = 0x1020
</code></pre>

<p>✅ Esto es seguro, no hay ningún acceso de memoria, solo una suma.</p>

<h3>3️⃣ <code>mov rax, [rbx + 20h]</code></h3>
<p>Aquí sí significa:<br>
“Lee el valor almacenado en la dirección (<code>rbx + 0x20</code>) y cópialo en <code>rax</code>”.</p>

<p>En este caso intenta leer de la dirección <code>0x0000000000001020</code>, que no pertenece a tu proceso.</p>

<p>💥 Windows lanza una excepción:</p>
<pre><code>0xC0000005: Infracción de acceso al leer la ubicación 0x0000000000001020
</code></pre>
...
<table border="1" cellpadding="6" cellspacing="0">
  <thead>
    <tr>
      <th>Instrucción</th>
      <th>Qué hace</th>
      <th>Accede memoria</th>
      <th>Resultado en tu caso</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>lea rax, [rbx+20h]</code></td>
      <td>Calcula dirección (no toca RAM)</td>
      <td>❌ No</td>
      <td>✅ rax = 0x1020</td>
    </tr>
    <tr>
      <td><code>mov rax, [rbx+20h]</code></td>
      <td>Lee desde memoria en esa dirección</td>
      <td>✅ Sí</td>
      <td>💥 Error: acceso inválido</td>
    </tr>
  </tbody>
</table>

<h2>✅ Ejemplo que muestra la diferencia sin error</h2>
<pre><code>.data
myVar dq 123456789ABCDEF0h

.code
TestFunction proc
    lea rbx, myVar          ; rbx = dirección de myVar
    lea rax, [rbx+20h]      ; solo calcula (rax = dirección+0x20)
    mov rcx, [rbx]          ; lee el valor contenido en myVar
    ret
TestFunction endp
end
</code></pre>

<p>🔸 Después de ejecutar:</p>
<ul>
  <li><strong>rax</strong> tendrá una dirección calculada (no usada realmente).</li>
  <li><strong>rcx</strong> tendrá el contenido almacenado en <code>myVar</code> (<code>123456789ABCDEF0h</code>).</li>
</ul>
...
<div class="seccion derecha">
    <p>Ejemplo 2: Con codigo notamos la diferencia</p>
    <img src="../../../img/guia_264.png" alt="" width="700">

 
    <h2>💡 “El propósito del <code>lea</code> es guardar la dirección para que la llamada sepa en qué dirección buscar”</h2>

<p>Déjame explicártelo paso a paso, con relación directa a tu ejemplo real, para que veas exactamente por qué y cuándo se hace así 👇</p>

<h3>🧩 1. Qué hace realmente <code>lea</code></h3>
<p><strong>LEA = Load Effective Address</strong></p>
<ul>
  <li>👉 No lee memoria.</li>
  <li>👉 Solo calcula una dirección (puntero).</li>
</ul>

<pre><code>lea rax, [rbp+input]</code></pre>
<p>significa:</p>
<p><em>“Guarda en <code>RAX</code> la dirección donde está la variable <code>input</code>.”</em></p>

<h3>🧩 2. Qué necesita una función como <code>getline</code></h3>
<pre><code>char input[64];
std::cin.getline(input, 64);
</code></pre>

<p>La función <code>getline()</code> necesita saber <strong>dónde guardar los caracteres</strong> que el usuario escriba.</p>
<ul>
  <li>👉 Necesita un puntero a un bloque de memoria (<code>char *</code>).</li>
  <li>❌ No necesita el valor de <code>input</code> (porque es un arreglo, no un número).</li>
</ul>

<h3>🧩 3. Cómo se traduce eso en ensamblador</h3>
<pre><code>lea     rax, [rbp+input]   ; RAX = dirección del buffer local "input"
mov     r8d, 40h           ; tercer argumento = tamaño del buffer
mov     rdx, rax           ; segundo argumento = puntero (char *)
mov     rax, cs:_refptr__ZSt3cin
mov     rcx, rax           ; primer argumento = std::cin
call    _ZNSi7getlineEPcx  ; llama a std::cin.getline(char*, long long)
</code></pre>

<h4>🔍 Qué ocurre conceptualmente</h4>
<table border="1" cellpadding="6" cellspacing="0">
  <thead>
    <tr>
      <th>Registro</th>
      <th>Qué contiene</th>
      <th>Qué representa en C++</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>RCX</td><td><code>std::cin</code></td><td>El objeto sobre el que se llama el método</td></tr>
    <tr><td>RDX</td><td>Dirección de <code>input</code></td><td>El <code>char*</code> donde escribir</td></tr>
    <tr><td>R8D</td><td>64</td><td>El tamaño del buffer</td></tr>
    <tr><td>CALL getline</td><td>—</td><td>Llama a la función y escribe dentro del buffer</td></tr>
  </tbody>
</table>

<p>👉 Sin el <code>lea</code>, la función no sabría dónde escribir los caracteres ingresados.</p>

<h3>🧠 4. Qué pasaría si usaras <code>mov</code> en lugar de <code>lea</code></h3>
<pre><code>mov rax, [rbp+input]
mov rdx, rax
</code></pre>
<p>Eso leería el contenido del primer byte de <code>input</code> (por ejemplo, 0x00 o 0x41),<br>
y luego lo usaría como si fuera una dirección de memoria.</p>

<p>💥 Resultado: excepción <code>0xC0000005 (Access Violation)</code>,<br>
porque la dirección <code>0x41</code> o <code>0x00</code> no pertenece a tu proceso.</p>

<h3>🧩 5. En términos de C/C++</h3>
<table border="1" cellpadding="6" cellspacing="0">
  <thead>
    <tr>
      <th>Código en C</th>
      <th>Código en ensamblador</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>char input[64];</code></td>
      <td><code>sub rsp, 64h</code> (reserva espacio en el stack)</td>
    </tr>
    <tr>
      <td><code>cin.getline(input, 64);</code></td>
      <td><code>lea rdx, [rbp+input]</code> → pasa la dirección</td>
    </tr>
  </tbody>
</table>

<p>Y si escribieras esto en C:</p>
<pre><code>std::cin.getline(&input[0], 64);
</code></pre>
<p>el <code>&</code> (“address of”) en C equivale exactamente a <code>lea</code> en ensamblador.</p>

<h3>🧩 6. En resumen visual</h3>
<table border="1" cellpadding="6" cellspacing="0">
  <thead>
    <tr>
      <th>Concepto</th>
  ...
  <tbody>
    <tr>
      <td>Obtener valor</td>
      <td><code>x</code></td>
      <td><code>mov rax, [rbp+x]</code></td>
      <td>Leer el contenido</td>
    </tr>
    <tr>
      <td>Obtener dirección</td>
      <td><code>&amp;x</code></td>
      <td><code>lea rax, [rbp+x]</code></td>
      <td>Calcular dónde está guardado</td>
    </tr>
  </tbody>
</table>

<h2>✅ Conclusión final</h2>
<p>El propósito de <code>lea</code> es <strong>obtener la dirección (puntero) de una variable o buffer</strong>, para pasársela a una función que necesita saber dónde leer o escribir datos en memoria.</p>
<p>Sin <code>lea</code>, las funciones no sabrían dónde encontrar (o colocar) la información.</p>

<hr>
<h2> 1. Tu idea en limpio</h2>
<p>Tú dices:</p>

<pre>mov rcx, cs:_refptr__ZSt3cin</pre>

<p> Eso parece más corto y directo, y de hecho <strong>a veces funciona</strong>.</p>

<h2> 2. Qué hace <code>cs:_refptr__ZSt3cin</code></h2>
<p>
<code>_refptr__ZSt3cin</code> es un puntero global que contiene la dirección del objeto <code>std::cin</code>.<br>
El prefijo <code>cs:</code> indica el segmento de código (en x64 casi no se usa, pero MSVC lo deja por compatibilidad).
</p>

<p>Es decir:</p>
<pre>mov rax, cs:_refptr__ZSt3cin</pre>

<p>
carga el <strong>contenido de esa dirección</strong> (no la dirección misma),  
o sea: el puntero real al objeto <code>cin</code>.
</p>

<h2> 3. Qué pasaría si hicieras <code>mov rcx, cs:_refptr__ZSt3cin</code></h2>
<p>
Eso también copiaría el puntero al objeto <code>cin</code> directamente en <code>rcx</code>,
y en principio sería equivalente a:
</p>
...
<p> Pero… hay un detalle técnico importante 👇</p>

<h2> 4. El motivo del paso intermedio por <code>RAX</code></h2>
<p>Los compiladores como <strong>MSVC</strong> tienden a usar registros intermedios por varias razones:</p>

<ul>
  <li><strong>Uniformidad de código:</strong> El compilador traduce las expresiones a pasos simples y consistentes.</li>
  <li><strong>Reutilización del valor:</strong> Si el puntero a <code>std::cin</code> se necesita más adelante, ya está en <code>RAX</code>.</li>
  <li><strong>Restricciones del ensamblador:</strong> Algunas combinaciones (<code>mov [mem], [mem]</code> o <code>mov reg, [rip+offset]</code>) no siempre son válidas directamente.</li>
  <li><strong>Compatibilidad con ASLR/PIE:</strong> A veces el acceso a símbolos globales debe pasar por un registro temporal antes de usarse.</li>
</ul>

<div class="highlight">
<b>Ejemplo de patrón típico del compilador:</b>
<pre>
mov rax, cs:_refptr__ZSt3cin
mov rcx, rax
</pre>
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>mov rcx, cs:_refptr__ZSt3cin</code></td>
      <td>Carga el puntero global directamente</td>
      <td>✅ Sí, funcional</td>
      <td>Más directa, pero no siempre generada automáticamente</td>
    </tr>
    <tr>
      <td><code>mov rax, cs:_refptr__ZSt3cin<br>mov rcx, rax</code></td>
      <td>Carga primero en <code>RAX</code> y luego pasa a <code>RCX</code></td>
      <td>✅ También válida</td>
      <td>Más genérica y segura según las reglas del compilador</td>
    </tr>
  </tbody>
</table>
