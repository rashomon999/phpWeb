...
<p><strong>ptr</strong> es la abreviación de <strong>pointer</strong> (puntero). Pero no indica una variable “puntero”, sino el tamaño del dato al que estás accediendo en memoria.</p>
<p>En otras palabras:</p>
<p>🔹 <strong>ptr</strong> le dice al ensamblador cuántos bytes debe leer o escribir en la dirección de memoria indicada.</p>

<h3> Ejemplo general</h3>
<pre><code>mov byte ptr [rbp-11h], 1</code></pre>

<p><strong>👉 Traducción:</strong></p>
<ul>
  <li><strong>[rbp-11h]</strong> → dirección en la pila.</li>
  <li><strong>byte ptr</strong> → el valor que hay ahí ocupa 1 byte.</li>
  <li><strong>mov ... , 1</strong> → guarda el número 1 en esa dirección.</li>
</ul>
<p>Sin <strong>byte ptr</strong>, el ensamblador no sabría si debe escribir 1 byte, 2, 4 u 8 bytes.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>8</td>
      <td>long long o int64_t</td>
    </tr>
    <tr>
      <td>10 bytes</td>
      <td><strong>tbyte ptr</strong></td>
      <td>10</td>
      <td>long double (a veces)</td>
    </tr>
  </tbody>
</table>

<h3>⚙️ Ejemplos reales</h3>
<p><strong>1️⃣ Leer 4 bytes (un entero)</strong></p>
<pre><code>mov eax, dword ptr [rbp-4]</code></pre>
<p>→ Carga en <strong>eax</strong> el entero de 4 bytes guardado en <strong>[rbp-4]</strong>.</p>

<p><strong>2️⃣ Escribir 1 byte (un carácter)</strong></p>
<pre><code>mov byte ptr [rbp-11h], 'A'</code></pre>
<p>→ Guarda la letra <strong>'A'</strong> (0x41) en la variable <strong>ch_0</strong>.</p>

<p><strong>3️⃣ Guardar un número de 64 bits</strong></p>
<pre><code>mov qword ptr [rbp-10h], 12345678h</code></pre>
<p>→ Guarda el valor <strong>0x12345678</strong> en una variable de 8 bytes (64 bits).</p>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>“Pointer” (puntero)</td>
    </tr>
    <tr>
      <td><strong>byte ptr, word ptr, dword ptr, qword ptr</strong></td>
      <td>Tamaño del dato (1, 2, 4, 8 bytes)</td>
    </tr>
    <tr>
      <td><strong>[rbp-XXh]</strong></td>
      <td>Dirección en la pila (no el valor literal)</td>
    </tr>
    <tr>
      <td><strong>mov byte ptr [rbp-11h], al</strong></td>
      <td>Mueve 1 byte del registro <strong>AL</strong> a la dirección <strong>[rbp-11h]</strong></td>
    </tr>
  </tbody>
</table>

    <hr>
    <!-- HTML sencillo sin estilos -->
<h3>🧩 Qué es rbp</h3>
<p><strong>rbp</strong> (<em>Base Pointer</em> o <em>Frame Pointer</em>) es un registro que guarda una dirección de memoria. Esa dirección marca un punto fijo dentro del stack (la “base” del <em>stack frame</em> de la función actual).</p>

<p><strong>👉 En otras palabras:</strong></p>
<p><strong>rbp</strong> no contiene datos, sino una dirección — el punto de referencia para acceder a las variables locales y parámetros.</p>

<h3>📚 Qué pasa al entrar en una función</h3>
<p>Cuando entras a una función en <strong>x86-64</strong>, el compilador casi siempre genera esto:</p>

<pre><code>push rbp        ; guarda el rbp anterior
mov  rbp, rsp   ; fija el nuevo rbp (inicio del frame actual)
sub  rsp, 80h   ; reserva espacio para variables locales
</code></pre>

<p>Esto hace que:</p>
<ul>
  <li><strong>[rbp]</strong> → contenga el rbp anterior (para restaurarlo al final)</li>
  <li><strong>[rbp + 8]</strong> → contenga la dirección de retorno</li>
  <li><strong>[rbp - ...]</strong> → sean las variables locales</li>
</ul>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Base Pointer (punto fijo dentro del marco de la función actual)</td>
    </tr>
    <tr>
      <td><strong>rsp</strong></td>
      <td>Stack Pointer (tope actual del stack, varía con push/pop)</td>
    </tr>
    <tr>
      <td><strong>“Inicio del stack”</strong></td>
      <td>No está definido por rbp, sino por el sistema operativo</td>
    </tr>
    <tr>
      <td><strong>Cada función</strong></td>
      <td>Tiene su propio rbp y su propio bloque de pila</td>
    </tr>
  </tbody>
</table>
...
<!-- HTML sencillo sin estilos -->
<h3>🧩 Lo que tienes</h3>
<p>Tu stack frame (según IDA) contiene algo como:</p>
...
<p>Así que cuando lees <strong>[rbp+input]</strong>, en realidad es <strong>[rbp - 0x60]</strong>.</p>

<h3>🔍 Análisis línea a línea</h3>
<pre><code>lea     rdx, [rbp+input]</code></pre>
<p>➡️ <strong>rdx ← dirección de input</strong><br>
Esto significa: “carga en rdx el puntero al inicio del buffer input”.<br>
<strong>rbp + input → rbp - 0x60 → dirección base del arreglo input.</strong></p>

<pre><code>mov     rax, [rbp+i]</code></pre>
<p>➡️ <strong>rax ← valor de la variable i</strong><br>
<i>i</i> está en <strong>[rbp - 0x10]</strong> y ocupa 8 bytes.<br>
Así que esto copia su valor (un número, no una dirección) a <strong>rax</strong>.</p>

<pre><code>add     rax, rdx</code></pre>
<p>➡️ <strong>rax = rdx + rax</strong> → suma la dirección base de input con el valor de i.<br>
Esto es equivalente a: <code>&amp;(input[i])</code><br>
Es decir, la dirección del carácter número i dentro del buffer.</p>

<pre><code>movzx   eax, byte ptr [rax]</code></pre>
<p>➡️ Carga el byte que está en esa dirección → <strong>input[i]</strong><br>
(<strong>movzx</strong> = mueve con extensión a cero, es decir, lo convierte a entero sin signo).</p>

<pre><code>mov     [rbp+ch_0], al</code></pre>
<p>➡️ Guarda ese byte en la variable local <strong>ch_0</strong>.<br>
(<strong>al</strong> es el registro de 8 bits bajos de <strong>rax</strong>).</p>

<h3>📘 Traducción a C aproximada:</h3>
<pre><code>ch_0 = input[i];</code></pre>

<pre><code>cmp     [rbp+i], 3
ja      short loc_14000158A</code></pre>
<p>➡️ Compara <strong>i</strong> con 3.<br>
Si <strong>i > 3</strong>, salta (branch).<br>
Significado: hay una condición tipo <code>if (i > 3) goto ...</code></p>

<p>Hasta aquí tienes que entender algo crucial:</p>
<p>Todas esas operaciones <strong>[rbp+input]</strong>, <strong>[rbp+i]</strong>, <strong>[rbp+ch_0]</strong> son accesos a variables locales dentro del stack frame actual.<br>
Y todo está referenciado respecto a <strong>rbp</strong> porque <strong>rbp</strong> es la base fija de la función.</p>

<h3>⚙️ Luego, el bloque siguiente</h3>
<pre><code>mov     edx, 0          ; __n
lea     rax, _ZL10claves_xor ; claves_xor
mov     rcx, rax        ; this
call    _ZNKSt5arrayIhLy3EEixEy ; std::array&lt;uchar,3ull&gt;::operator[](ulong long)
</code></pre>
<p>Esto llama a <strong>std::array&lt;unsigned char, 3&gt;::operator[](unsigned long)</strong>, o sea:</p>
<pre><code>claves_xor[0]</code></pre>
<p>ya que <strong>edx = 0</strong>.</p>

<pre><code>movzx   eax, byte ptr [rax]
xor     al, [rbp+ch_0]
mov     [rbp+tmp], al</code></pre>
<p>➡️ XOR entre el valor de <strong>claves_xor[0]</strong> y <strong>ch_0</strong>.<br>
Guarda el resultado en <strong>tmp</strong>.<br>
<code>tmp = claves_xor[0] ^ ch_0;</code></p>

<pre><code>mov     rax, [rbp+i]
mov     rdx, rax        ; __n
lea     rax, _ZL5clave  ; clave
mov     rcx, rax        ; this
call    _ZNKSt5arrayIhLy12EEixEy ; std::array&lt;uchar,12ull&gt;::operator[](ulong long)
...
<div class="seccion derecha">
  ...
  <p><strong>Traducción aproximada:</strong></p>
<pre><code>if (tmp == clave[i])
    goto loc_140001613;
</code></pre>

<h3> Qué te enseña esto sobre rbp</h3>
<p>Esto lo deja clarísimo:</p>

<table>
  <thead>
    <tr>
      <th>Expresión ensamblador</th>
      <th>Qué significa conceptualmente</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><strong>[rbp+input]</strong></td>
      <td>Dirección base del arreglo local <strong>input</strong></td>
    </tr>
    <tr>
      <td><strong>[rbp+i]</strong></td>
      <td>Valor de la variable local <strong>i</strong></td>
    </tr>
    <tr>
      <td><strong>[rbp+ch_0]</strong></td>
      <td>Variable local <strong>ch_0</strong></td>
    </tr>
    <tr>
      <td><strong>[rbp+tmp]</strong></td>
      <td>Variable local <strong>tmp</strong></td>
    </tr>
    <tr>
      <td><strong>rbp</strong></td>
      <td>Punto fijo dentro del stack de la función (referencia para todo lo anterior)</td>
    </tr>
  </tbody>
</table>

<hr>
<h3>🧩 Parámetros y registros</h3>
<table>
  <thead>
    <tr>
      <th>Parámetro</th>
  ...
  <tbody>
    ...
    <tr>
      <td>5° y siguientes</td>
      <td>Se ponen en la <strong>pila</strong> (stack)</td>
    </tr>
  </tbody>
</table>

<h3>📘 Qué pasa justo antes del <code>call</code></h3>
<p>Antes de llamar a la función, el código carga los valores de los argumentos en esos registros.</p>

<p><strong>Ejemplo:</strong></p>
<pre><code>mov     rcx, rax     ; primer argumento (this)
mov     edx, 0       ; segundo argumento (__n)
call    _ZNKSt5arrayIhLy3EEixEy
</code></pre>

<p>➡️ Cuando ocurre el <strong>call</strong>, la CPU no ve parámetros en el código fuente, pero la función que se llama recibirá:</p>
<pre><code>operator[](this = rax, index = 0)</code></pre>

<p><strong>Porque:</strong></p>
<ul>
  <li><strong>rcx</strong> tenía el puntero al objeto (<code>this</code>).</li>
  <li><strong>edx</strong> tenía el índice <code>0</code>.</li>
</ul>

<hr>
<p>El <strong>stack pointer</strong> (puntero de pila) es un registro especial de la CPU que apunta siempre a la cima de la pila (stack).</p>

<h3>🔹 Conceptos clave</h3>

<p>En <strong>x86-64</strong>, el registro que actúa como stack pointer se llama <strong>RSP</strong>.</p>

<p>La pila (stack) es una región de memoria temporal usada para:</p>
<ul>
  <li>Guardar <strong>direcciones de retorno</strong> al llamar funciones.</li>
  <li>Guardar <strong>registros</strong> que necesitan preservarse.</li>
  <li>Almacenar <strong>variables locales</strong> de una función.</li>
</ul>

<p>El stack pointer siempre indica dónde se encuentra el siguiente espacio libre en la pila.</p>

<h3>Instrucciones que   modifican el stack pointer (RSP) </h3>

<p>Solo <strong>estas instrucciones cambian físicamente el stack</strong> (es decir, modifican <strong>RSP</strong> y por tanto hacen crecer o decrecer la pila):</p>

<p><strong>push</strong>: <code>push rax</code> → <code>RSP -= 8</code>, y guarda <code>rax</code> en <code>[RSP]</code></p>
<p><strong>pop</strong>: <code>pop rbx</code> → lee <code>[RSP]</code> a <code>rbx</code> y <code>RSP += 8</code></p>
<p><strong>call</strong>: <code>call func</code> → <code>RSP -= 8</code> y guarda dirección de retorno</p>
<p><strong>ret</strong>: <code>ret</code> → lee dirección de retorno de <code>[RSP]</code>, luego <code>RSP += 8</code></p>
<p><strong>sub/add rsp, X</strong>: <code>sub rsp, 80h</code> → reserva (sub) o libera (add) espacio local</p>

<h3>Instrucciones que  NO cambian el stack pointer </h3>

<p><strong>mov</strong>: <code>mov [rbp-10h], rax</code> → Escribe <strong>dentro del stack frame</strong>, pero <strong>no cambia RSP</strong></p>
<p><strong>lea</strong>: <code>lea rax, [rbp-60h]</code> → Calcula una dirección (por ejemplo, un puntero a una variable local)</p>
<p><strong>cmp, xor, etc.</strong>: Operan sobre registros o memoria, pero <strong>no tocan RSP</strong></p>

<h3>Entonces:</h3>
<p><code>push</code>, <code>pop</code>, <code>call</code>, <code>ret</code>, y <code>sub/add rsp</code> → <strong>sí alteran</strong> la altura de la pila.</p>
<p><code>mov</code>, <code>lea</code>, <code>cmp</code>, <code>xor</code>, etc. → <strong>solo usan la pila existente</strong>, <strong>no la expanden ni reducen</strong>.</p>


<hr>
<strong>analisis una de las primeras instrucciones</strong>
<h3>1️⃣ Instrucción</h3>
<pre><code>add rsp, 0FFFFFFFFFFFFFF80h</code></pre>

<p><strong>rsp</strong> = registro del stack pointer (tope de la pila).</p>
<p><strong>0FFFFFFFFFFFFFF80h</strong> = número hexadecimal de 64 bits.</p>

<h3>2️⃣ Convertir el número a decimal</h3>
<p><strong>0xFFFFFFFFFFFFFF80</strong> es un número negativo en complemento a dos:</p>
<ul>
  <li>Resta 1: <strong>0xFFFFFFFFFFFFFF7F</strong></li>
  <li>Invertir bits → <strong>0x80</strong></li>
  <li>Decimal = -128</li>
</ul>

<p>✅ Por lo tanto: <strong>0xFFFFFFFFFFFFFF80 = -128 (decimal)</strong></p>

<h3>3️⃣ Qué hace la instrucción</h3>
<pre><code>add rsp, -128</code></pre>

<p>Es equivalente a:</p>
<pre><code>sub rsp, 128</code></pre>

<ul>
  <li>Reserva 128 bytes en la pila para variables locales.</li>
  <li>Se mueve el stack pointer hacia abajo (en x86-64, la pila crece hacia direcciones más bajas).</li>
</ul>


<h3>Representación clara del Stack Frame</h3>
<pre>
Dirección ↑
─────────────────────────────
| <strong>Valor de RBP anterior</strong>     | ← [RBP] (push rbp)
─────────────────────────────
| <strong>Dirección de retorno</strong>      | ← [RBP + 8]
─────────────────────────────
|   ↑                       |
|   |  Área local (128 bytes reservados)  
|   ↓                       |
|---------------------------|
| <strong>input (64 bytes)</strong>          | ← [RBP - 0x60] ... [RBP - 0x21]
|---------------------------|
| <strong>tmp (1 byte)</strong>              | ← [RBP - 0x12]
|---------------------------|
| <strong>ch_0 (1 byte)</strong>             | ← [RBP - 0x11]
|---------------------------|
| <strong>i (8 bytes, qword)</strong>       | ← [RBP - 0x10] ... [RBP - 0x9]
|---------------------------|
| <strong>len (8 bytes, qword)</strong>     | ← [RBP - 0x8]  ... [RBP - 0x1]
─────────────────────────────
|                           |
|      <strong>Pila libre ↓</strong>         |
─────────────────────────────
Direcciones ↓
</pre>
    ...
    <hr>
     <h1>ANÁLISIS DETALLADO DE CADA SALTO</h1>

    <section>
        <h2>A) jbe - Jump if Below or Equal</h2>
        <pre><code>cmp     [rbp+len], 0Bh       ; Comparar len con 11
jbe     loc_140001625        ; Saltar si len ≤ 11</code></pre>
        <p><strong>Condición:</strong> Salta si len ≤ 11 (comparación sin signo)</p>
        <p><strong>Flags que revisa:</strong> CF=1 OR ZF=1</p>
        <p><strong>Uso en el código:</strong></p>
        <ul>
            <li>Si len ≤ 11 → Password muy corto → FALLO</li>
            <li>Si len > 11 → Continuar verificación</li>
        </ul>
        <div class="example">
            <p>Ejemplo:</p>
            <p>len = 10 → 10 ≤ 11 → jbe SALTA → Acceso denegado</p>
            <p>len = 12 → 12 ≤ 11 → NO (12 > 11) → Continuar</p>
        </div>
    </section>

    <section>
        <h2>B) ja - Jump if Above</h2>
        <pre><code>cmp     [rbp+i], 3           ; Comparar i con 3
ja      short loc_14000158A    ; Saltar si i > 3</code></pre>
        <p><strong>Condición:</strong> Salta si i > 3 (comparación sin signo)</p>
        <p><strong>Flags que revisa:</strong> CF=0 AND ZF=0</p>
        <p><strong>Uso en el código:</strong></p>
        <ul>
            <li>Si i ≤ 3 → Usar claves_xor[0]</li>
            <li>Si i > 3 → Saltar a probar otras claves</li>
        </ul>
        <div class="example">
            <p>Ejemplo:</p>
            <p>i = 2 → 2 > 3 → NO → Usa claves_xor[0]</p>
            <p>i = 5 → 5 > 3 → SÍ → ja SALTA → Probar claves_xor[1]</p>
        </div>
    </section>
