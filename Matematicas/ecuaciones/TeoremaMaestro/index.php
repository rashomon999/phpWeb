...
<div class="seccion izquierda">
    ...
    <h2>Uso del Método Maestro</h2>
  <p>Para usar el método maestro, simplemente determinamos cuál caso (si alguno) del teorema maestro aplica y escribimos la solución.</p>

  <p>Como primer ejemplo, consideremos:</p>
  <p>\( T(n) = 9T(n/3) + n \)</p>
  <p>Para esta recurrencia, tenemos \( a = 9 \), \( b = 3 \), \( f(n) = n \), y por lo tanto \( n^{\log_b a} = n^{\log_3 9} = \Theta(n^2) \). Como \( f(n) = O(n^{\log_3 9 - \varepsilon}) \), donde \( \varepsilon = 1 \), podemos aplicar el <strong>caso 1</strong> del teorema maestro y concluir que la solución es \( T(n) = \Theta(n^2) \).</p>

  <p>Ahora consideremos:</p>
  <p>\( T(n) = T(2n/3) + 1 \)</p>
  <p>Aquí, \( a = 1 \), \( b = 3/2 \), \( f(n) = 1 \), y \( n^{\log_b a} = n^0 = 1 \). Aplica el <strong>caso 2</strong>, ya que \( f(n) = \Theta(n^{\log_b a}) = \Theta(1) \), y por lo tanto la solución a la recurrencia es \( T(n) = \Theta(\log n) \).</p>

  <p>Para la recurrencia:</p>
  <p>\( T(n) = 3T(n/4) + n \log n \)</p>
  <p>Tenemos \( a = 3 \), \( b = 4 \), \( f(n) = n \log n \), y \( n^{\log_b a} = n^{\log_4 3} = O(n^{0.793}) \). Como \( f(n) = \Omega(n^{\log_4 3 + \varepsilon}) \), donde \( \varepsilon \approx 0.2 \), aplica el <strong>caso 3</strong> si podemos demostrar que se cumple la condición de regularidad para \( f(n) \).</p>
  <p>Para \( n \) suficientemente grande, tenemos que:</p>
  <p>\( a f(n/b) = 3(n/4)\log(n/4) \approx (3/4)n \log n = c f(n) \), con \( c = 3/4 \).</p>
  <p>En consecuencia, por el <strong>caso 3</strong>, la solución es \( T(n) = \Theta(n \log n) \).</p>

  <p>El método maestro <em>no</em> aplica a la recurrencia:</p>
  <p>\( T(n) = 2T(n/2) + n \log n \)</p>
  <p>Aquí, \( a = 2 \), \( b = 2 \), \( f(n) = n \log n \), y \( n^{\log_b a} = n \). Aunque \( f(n) = \omega(n) \), no es polinomialmente mayor que \( n \), ya que \( f(n)/n = \log n \), lo cual crece más lento que \( n^\varepsilon \) para cualquier \( \varepsilon > 0 \). Por lo tanto, esta recurrencia cae en el vacío entre los casos 2 y 3. (Ver el Ejercicio 4.6-2 para una solución.)</p>

  <h3>Ejemplos de las Secciones 4.1 y 4.2</h3>

  <p>Recurrencia (4.7):</p>
  <p>\( T(n) = 2T(n/2) + \Theta(n) \)</p>
  <p>Esta caracteriza el tiempo de ejecución del algoritmo divide y vencerás tanto para el problema del subarreglo máximo como para merge sort. Aquí, \( a = 2 \), \( b = 2 \), \( f(n) = \Theta(n) \), y \( n^{\log_b a} = n \). Aplica el <strong>caso 2</strong>, por lo que \( T(n) = \Theta(n \log n) \).</p>

  <p>Recurrencia (4.17):</p>
  <p>\( T(n) = 8T(n/2) + \Theta(n^2) \)</p>
  <p>Describe el tiempo de ejecución del primer algoritmo divide y vencerás que vimos para multiplicación de matrices. Aquí, \( a = 8 \), \( b = 2 \), \( f(n) = \Theta(n^2) \), y \( n^{\log_b a} = n^3 \). Como \( f(n) = O(n^3) \), aplica el <strong>caso 1</strong> y \( T(n) = \Theta(n^3) \).</p>

  <p>Finalmente, consideremos la recurrencia (4.18):</p>
  <p>\( T(n) = 7T(n/2) + \Theta(n^2) \)</p>
  <p>Describe el tiempo de ejecución del algoritmo de Strassen. Aquí, \( a = 7 \), \( b = 2 \), \( f(n) = \Theta(n^2) \), y \( n^{\log_b a} = n^{\log_2 7} \). Reescribiendo \( \log_2 7 \) como \( \lg 7 \), y recordando que \( 2.80 < \lg 7 < 2.81 \), vemos que \( f(n) = O(n^{\lg 7}) \) para \( \varepsilon \approx 0.8 \). De nuevo, aplica el <strong>caso 1</strong> y la solución es \( T(n) = \Theta(n^{\lg 7}) \).</p>
