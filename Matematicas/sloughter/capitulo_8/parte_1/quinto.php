...
<div class="seccion izquierda">
    ...
    <h2>(b) Tasa de crecimiento decreciente</h2>
    <p><strong>Supuesto:</strong></p>
    <p>\[
    k(t) = 0.01 \cdot \left(1 + 1.1 e^{-t/30} \right)
    \]</p>

    <p>Como no se puede resolver analíticamente, se usa el método de Runge-Kutta de segundo orden (RK2) con paso \( h = 0.1 \).</p>

    <h3>Algoritmo RK2:</h3>
    <p>\[
    m = \frac{h}{2} f(P_k, t_k), \quad
    P_{k+1} = P_k + h \cdot f(P_k + m, t_k + \frac{h}{2})
    \]</p>

    <h3>Código en Python:</h3>
    <pre><code>
import numpy as np
...
def k_variable(t):
    return 0.01 * (1 + 1.1 * np.exp(-t / 30))

def f(P, t):
    return k_variable(t) * P

for i in range(n):
    t = t_rk[i]
    P = P_rk[i]
    m = (h / 2) * f(P, t)
    P_next = P + h * f(P + m, t + h / 2)
    t_rk.append(t + h)
    P_rk.append(P_next)
...
P_rk = np.array(P_rk)
    ...
    <h3>Años estimados:</h3>
    <ul>
        <li>Para \( P(t) = 1000 \): año ≈ 1998.1</li>
        <li>Para \( P(t) = 2000 \): año ≈ 2047.1</li>
        <li>Para \( P(t) = 3000 \): año ≈ 2084.2</li>
    </ul>

    <hr>

    <h2>(c) Comparación gráfica</h2>

    <h3>Código en Python:</h3>
    <pre><code>
t_values = np.linspace(0, 100, 1001)
P_exact = P0 * np.exp(0.021 * t_values)

plt.figure(figsize=(10, 6))
plt.plot(t_values + 1990, P_exact, label="(a) Crecimiento constante (exacto)", color="blue")
plt.plot(t_rk + 1990, P_rk, label="(b) RK2 con k(t) decreciente", color="red", linestyle="--")
...
plt.title("Comparación del crecimiento poblacional en India (1990–2090)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
    ...
    <h3>Conclusión:</h3>
    <p>El modelo con tasa constante predice un crecimiento mucho más rápido. El modelo con \( k(t) \) decreciente muestra crecimiento desacelerado, alcanzando 2 y 3 mil millones varias décadas más tarde. Esto refleja mejor los efectos sociales, económicos y políticos reales a largo plazo.</p>
