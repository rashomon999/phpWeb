...
<div class="seccion izquierda">
    ...
    <p> 
    se llama una ecuación diferencial de primer orden. Resolver tal ecuación implica más que manipulación 
    algebraica; de hecho, aunque la ecuación misma involucra tres cantidades, \( x \), \( t \) y \( \dot{x} \), 
    para encontrar una solución debemos identificar una función \( x \), definida únicamente en términos de la
    variable independiente \( t \), que satisfaga la relación de (8.1.1) para todo \( t \) en algún intervalo 
    abierto. Para muchas ecuaciones, encontrar una solución exacta no es posible y tenemos que recurrir a 
    aproximaciones. En este capítulo discutiremos técnicas para encontrar tanto soluciones aproximadas como, 
    ...
    <p>
    con la condición inicial \( x(t_0) = x_0 \). Para simplificar la notación, con frecuencia omitiremos la
    variable independiente al referirnos a \( x(t) \) y escribiremos simplemente
    </p>
    ...
    <p>
    Ahora bien, si \( f(x, t) \) depende solo del valor de \( t \), es decir, si \( f(x, t) = g(t) \) para
    todos los valores de \( x \), donde \( g \) es una función únicamente de \( t \), entonces podemos
    resolver (8.1.3) mediante integración. Es decir, al integrar ambos lados de (8.1.3) obtenemos
    </p>

    <p style="text-align: center;">
    \[\int_{t_0}^{t} \dot{x}(s)\, ds = \int_{t_0}^{t} g(s)\, ds. \tag{8.1.4}\]
    </p>
    ...
    <p style="text-align: center;">
    \[x(t) = x(t_0) + \int_{t_0}^{t} g(s)\, ds. \tag{8.1.5}\]
    </p>
...
<p style="text-align: center;">
\[
x(t) = 5 + \int_{0}^{t} 4 \sin(3s)\, ds  \]
\[ = 5 - \frac{4}{3} \cos(3s)\bigg|_0^t \]
\[= 5 - \frac{4}{3} \cos(3t) + \frac{4}{3}  \]
\[= \frac{1}{3}(19 - 4 \cos(3t))   \]
</p>

<p>
Más generalmente, supongamos que \( f \) en (8.1.3) depende tanto de \( x \) como de \( t \). En ese caso, dado que el lado derecho de la ecuación involucra la función desconocida \( x \), no podemos simplemente integrar ambos lados de la ecuación. Ya hemos resuelto algunas ecuaciones como esta en secciones anteriores, como el modelo de crecimiento poblacional inhibido:
</p>
...
<p>
en la Sección 6.3, y discutiremos técnicas generales para encontrar soluciones exactas para varios tipos de ecuaciones en las secciones siguientes. Sin embargo, en esta sección nos concentraremos en técnicas numéricas directas para aproximar soluciones. En efecto, sabiendo lo difícil que puede ser evaluar integrales definidas ordinarias, no es difícil creer que muchas, si no la mayoría, de las ecuaciones diferenciales pueden resolverse solo mediante aproximación numérica.
</p>

<p>
Aunque la función \( x \) en la ecuación (8.1.3) es desconocida, tenemos suficiente información para encontrar su mejor aproximación afín en \( t_0 \). Es decir, la mejor aproximación afín a \( x \) en \( t_0 \) es
</p>

<p style="text-align: center;">
\[
T(t) = x_0 + \dot{x}(t_0)(t - t_0) = x_0 + f(x_0, t_0)(t - t_0). \tag{8.1.7}
\]
</p>
...
<p style="text-align: center;">
\[
x_1 = x_0 + h f(x_0, t_0) \tag{8.1.10}
\]
</p>

<p>
proporcionará una buena aproximación de \( x(t_0 + h) \). Sin embargo, queremos hacer más que esto; dado que \( x \) es una función, queremos poder aproximar sus valores sobre todo un intervalo, digamos \([t_0, t_1]\). Para hacerlo, elegimos un valor pequeño para \( h \) e iteramos el proceso que nos dio \( x_1 \). Específicamente, tomamos \( s_k = t_0 + kh \), para \( k = 0, 1, \ldots, n \), donde \( n \) se elige lo suficientemente grande como para que \( s_n \geq t_1 \), y calculamos
</p>
...
<p>
para \( k = 0, 1, \ldots, n - 1 \). Es decir, calculamos una aproximación para \( x(s_{k+1}) \) aplicando la mejor aproximación afín a nuestra aproximación para \( x(s_k) \), repitiendo el proceso suficientes veces hasta que hayamos aproximado \( x \) sobre todo el intervalo \([t_0, t_1]\). Este método de aproximación se conoce como el <strong>método de Euler</strong>.
</p>
...
<p>
con la condición inicial \( x(t_0) = x_0 \) en un intervalo \([t_0, t_1]\), elige un valor pequeño \( h > 0 \) y un entero \( n \) tal que \( t_0 + nh \geq t_1 \). Definiendo \( s_k = t_0 + kh \), para \( k = 0, 1, \ldots, n \), calcula \( x_1, x_2, \ldots, x_n \) usando la ecuación en diferencias:
</p>

<p style="text-align: center;">
\[
x_{k+1} = x_k + h f(x_k, s_k). \tag{8.1.12}
\]
</p>
...
<p>
Nótese que (8.1.12) hace uso de una ecuación en diferencias, una ecuación en tiempo discreto que encontramos por primera vez en la Sección 1.4, para aproximar la solución de una ecuación diferencial.
</p>

<h2>Ejemplo 8.1.2</h2>

<p>
Considera la ecuación diferencial \( \dot{x} = 0.04x \) con condición inicial \( x(0) = 40 \). Por nuestro trabajo en el Capítulo 6 sabemos que la solución de esta ecuación es:
</p>
...
<p>
En particular, \( x(50) = 40e^2 = 295.56 \), redondeando a dos cifras decimales. Para aproximar \( x \) en el intervalo \([0, 50]\) usando el método de Euler, tomaremos primero \( h = 1 \) y, comenzando con \( x_0 = 40 \), calculamos \( x_1, x_2, \ldots, x_{50} \), donde en este caso \( x_k \) aproximará \( x(k) \). Usando (8.1.12) con \( f(x, t) = 0.04x \) y redondeando a dos cifras decimales, tenemos:
</p>

<ul>
  <li>\( x_1 = x_0 + h(0.04x_0) = 40 + (1)(0.04)(40) = 40 + 1.6 = 41.60 \),</li>
  <li>\( x_2 = x_1 + h(0.04x_1) = 41.60 + (1)(0.04)(41.60) = 41.60 + 1.66 = 43.26 \),</li>
  <li>\( x_3 = x_2 + h(0.04x_2) = 43.26 + (1)(0.04)(43.26) = 43.26 + 1.73 = 44.99 \),</li>
</ul>

<p>
y así sucesivamente, terminando con \( x_{50} = 284.27 \). La siguiente tabla da los valores de \( x_k \) y \( x(t) \) para \( t = 0, 5, 10, \ldots, 50 \):
</p>
...
<table border="1" cellpadding="6" cellspacing="0">
  ...
  <tbody>
    <tr><td>0</td><td>40.00</td><td>40.00</td></tr>
    <tr><td>5</td><td>48.67</td><td>48.86</td></tr>
    <tr><td>10</td><td>59.21</td><td>59.67</td></tr>
    <tr><td>15</td><td>72.04</td><td>72.88</td></tr>
    <tr><td>20</td><td>87.64</td><td>89.02</td></tr>
    <tr><td>25</td><td>106.63</td><td>108.73</td></tr>
    <tr><td>30</td><td>129.74</td><td>132.80</td></tr>
    <tr><td>35</td><td>157.84</td><td>162.21</td></tr>
    <tr><td>40</td><td>192.04</td><td>198.12</td></tr>
    <tr><td>45</td><td>233.65</td><td>241.99</td></tr>
    <tr><td>50</td><td>284.27</td><td>295.56</td></tr>
  </tbody>
</table>

<p>
Observa que el error en nuestra aproximación, es decir, la diferencia entre \( x_k \) y \( x(k) \), aumenta a medida que \( k \) crece. Por ejemplo:
</p>

<ul>
  <li>\( x(5) - x_5 = 0.19 \),</li>
  <li>\( x(50) - x_{50} = 11.29 \).</li>
</ul>

<p>
Esto debe esperarse, ya que el error de la aproximación en cada paso se acumula con los errores cometidos en los pasos anteriores. La única forma de controlar la cantidad de error en nuestras aproximaciones es reducir el tamaño del paso. Por ejemplo, si reducimos el tamaño del paso a \( h = 0.1 \), obtenemos:
</p>

<ul>
  <li>\( x_1 = x_0 + h(0.04x_0) = 40 + (0.1)(0.04)(40) = 40 + 0.16 = 40.1600 \),</li>
  <li>\( x_2 = x_1 + h(0.04x_1) = 40.16 + (0.1)(0.04)(40.16) = 40.16 + 0.1606 = 40.3206 \),</li>
  <li>\( x_3 = x_2 + h(0.04x_2) = 40.3206 + (0.1)(0.04)(40.3206) = 40.3206 + 0.1613 = 40.4819 \),</li>
</ul>

<p>
y así sucesivamente, terminando con \( x_{500} = 294.39 \) como nuestra aproximación para \( x(50) \). En general, con \( h = 0.1 \), \( x_k \) es una aproximación para \( x(0 + kh) = x(0.1k) \). De forma equivalente, \( x(t) \) se aproxima mediante \( x_k \) donde \( k = 10t \), siempre que \( 10t \) sea un número entero.
</p>

<p>
La siguiente tabla da los valores de \( x_{10t} \) y \( x(t) \) para \( t = 0, 5, 10, \ldots, 50 \):
</p>

    <img src="../../../../img/approx.png" alt="">
    <img src="../../../../img/table.png" alt="">

    <p>
Como era de esperarse, reducir el tamaño del paso de \( h = 1 \) a \( h = 0.1 \) reduce considerablemente el error en las aproximaciones. La Figura 8.1.1 muestra las gráficas de ambas aproximaciones de \( x \) (la gráfica de \( x \) no se muestra ya que, a esta escala, es esencialmente la misma que nuestra segunda aproximación).
</p>

    <hr>
    <strong>paretesis</strong>
    <p>
La expresión:
</p>
...
<p>
es la clave para pasar de la forma general de la expansión de Taylor:
</p>

<p style="text-align: center;">
\[
x(t) = x_0 + f(x_0, t_0)(t - t_0) + \mathcal{O}((t - t_0)^2)
\]
</p>
...
<p style="text-align: center;">
\[
x(t_0 + h) = x_0 + h f(x_0, t_0) + \mathcal{O}(h^2)
\]
</p>

<p><strong>Entonces, ¿por qué lo hacen?</strong></p>

<p>
Porque en métodos numéricos como el <em>método de Euler</em>, se da un paso de tamaño fijo \( h \), y se itera:
</p>

<p style="text-align: center;">
\[
t_0,\ t_0 + h,\ t_0 + 2h,\ t_0 + 3h,\ \ldots
\]
</p>

<p>
Por lo tanto, expresar todo en términos de \( h \) en lugar de \( t - t_0 \) simplifica el análisis y la implementación.
</p>
