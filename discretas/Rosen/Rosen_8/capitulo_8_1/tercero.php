...
<div class="seccion izquierda">
    ...
    <p>Ahora que hemos desarrollado esta relación de recurrencia, podemos construir un algoritmo eficiente, el <strong>Algoritmo 1</strong>, para calcular el número máximo total de asistentes. Aseguramos que el algoritmo sea eficiente almacenando el valor de cada \(T(j)\) después de calcularlo. Esto nos permite computar \(T(j)\) solo una vez. Si no lo hiciéramos, el algoritmo tendría una complejidad exponencial en el peor caso. Este proceso de almacenamiento de valores ya calculados se llama <strong>memoización</strong> y es una técnica importante para hacer que los algoritmos recursivos sean eficientes.</p>

<h3>ALGORITMO 1 – Algoritmo de Programación Dinámica para Programación de Charlas</h3>

<pre>
procedimiento AsistenciaMáxima (s₁, s₂,…, sₙ: tiempos de inicio de charlas;
                                 e₁, e₂,…, eₙ: tiempos de fin de charlas;
                                 w₁, w₂,…, wₙ: número de asistentes por charla)

  ordenar las charlas por tiempo de finalización y renumerar de forma que e₁ ≤ e₂ ≤ ⋯ ≤ eₙ
  
  para j := 1 hasta n hacer
    si no existe charla i con i < j compatible con la charla j
      p(j) := 0
    si no
      p(j) := máximo i tal que i < j y la charla i es compatible con la charla j
  
  T(0) := 0
  para j := 1 hasta n hacer
    T(j) := max(w_j + T(p(j)), T(j - 1))

  devolver T(n)  {T(n) es el número máximo de asistentes}
</pre>

<p>En el Algoritmo 1 determinamos el número máximo de asistentes que puede lograrse con una programación de charlas, pero no encontramos una programación que lo logre. Para encontrar las charlas que deben ser programadas, usamos el hecho de que la charla \(j\) pertenece a una solución óptima para las primeras \(j\) charlas si y solo si:</p>

<p style="text-align: center;">\( w_j + T(p(j)) \geq T(j - 1) \)</p>

<p>Se deja como el Ejercicio 53 construir un algoritmo basado en esta observación que determine qué charlas deben programarse para lograr el número máximo total de asistentes.</p>

<p>El Algoritmo 1 es un buen ejemplo de programación dinámica, ya que el total máximo de asistencia se encuentra usando las soluciones óptimas de subproblemas superpuestos, cada uno de los cuales determina el máximo total de asistencia de las primeras \(j\) charlas para algún \(1 \leq j \leq n - 1\).</p>

<p>Consulta los Ejercicios 56 y 57, y los Ejercicios Suplementarios 14 y 17 para otros ejemplos de programación dinámica.</p>
