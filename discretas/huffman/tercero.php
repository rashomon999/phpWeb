...
<div class="seccion izquierda">
    <form action="./index.php" method="POST" onsubmit="handleSubmit(event)" autocomplete="off">
      <h4>🔁 ¿Por qué suma total?</h4>
  <p>Porque al combinar todos los nodos, al final el nodo raíz tiene la suma acumulada de todos los pesos anteriores. Así:</p>

  <div class="code-block">
Raíz (100)
├── izquierda: subárbol con frecuencia 55
└── derecha: subárbol con frecuencia 45
  </div>

  <h4> Resumen:</h4>
  <ul>
    <li>✅ La raíz es un nodo interno, no es un símbolo real.</li>
    <li>✅ Su peso es la suma total de todas las frecuencias originales.</li>
    <li>✅ Desde la raíz se codifican los bits: <strong>0 para izquierda</strong>, <strong>1 para derecha</strong>.</li>
  </ul>

  <p>¿Querés que te dibuje un árbol de Huffman paso a paso con un ejemplo?</p>
  <hr>
  <h4>Analisis merge</h4>
    <h2> ¿La lista ya está ordenada?</h2>
  <p>
    Sí, la lista que llega a <code>insertInOrder</code> está ordenada por frecuencia cuando se llama desde <code>mergeCombine</code>.
    Pero al crear un nuevo nodo (combinación de dos menores), ese nuevo nodo debe insertarse en el lugar correcto para mantener el orden.
  </p>

  <h2> ¿Por qué necesitas <code>insertInOrder</code>?</h2>
  <p>
    Porque al combinar los dos nodos de menor frecuencia, se genera un nuevo nodo con frecuencia combinada. Este nodo se debe insertar
    en su posición correcta en la lista ya ordenada para no romper el orden.
  </p>

  <h3>💡 Ejemplo:</h3>

  <p>Supón que tienes esta lista de nodos hoja ordenada por frecuencia:</p>
  <pre><code>
val lista = List(
  Leaf('c', 1),
  Leaf('d', 5),
  ...
  <p>El algoritmo toma los dos menores: <code>1</code> y <code>5</code>, y crea un nodo combinado:</p>
  <pre><code>
val merged = InternalNode(Leaf('c', 1), Leaf('d', 5)) // freq = 6
  </code></pre>

  <p>Ahora, <code>rest = List(Leaf('e', 7), Leaf('f', 12))</code> sigue ordenado. Debemos insertar <code>merged</code> en la posición correcta:</p>

  <pre><code>
val updated = insertInOrder(rest, merged)
  </code></pre>

  <p>Resultado esperado:</p>
  <pre><code>
List(
  InternalNode(..., ...) con freq 6,
  Leaf('e', 7),
  Leaf('f', 12)
)
  </code></pre>
    ...
    <h2>✅ Resumen:</h2>
    <ul>
    <li>La lista está ordenada al inicio.</li>
    <li>Al combinar dos nodos, el resultado (merged) debe insertarse sin romper ese orden.</li>
    <li>Ahí es donde entra <code>insertInOrder</code>.</li>
    </ul>
    <hr>
    <h3>Complejidad</h3>
    <div>
...
def build(symbols: List[(Char, Int)]): HuffmanNode = { 
  val leaves = symbols.map { case (c, f) => Leaf(c, f): HuffmanNode } O(n)
  mergeCombine(leaves)  O(n^2) 
}
  </code></pre>

  <h3> ¿Qué hace esta función?</h3>
  <p><code>symbols.map { ... }</code><br>
  Convierte cada tupla <code>(Char, Int)</code> en una hoja del árbol de Huffman:</p>

  <pre><code class="language-scala">
Leaf(c, f): HuffmanNode
  </code></pre>

  <p>✅ Esto se hace en tiempo lineal, porque recorre la lista una sola vez.</p>
  <p> <strong>Complejidad:</strong> <code>O(n)</code></p>

  <p><code>mergeCombine(leaves)</code><br>
  Construye el árbol de Huffman combinando nodos recursivamente.</p>

  <p>Aquí está el trabajo pesado, así que veamos su complejidad 👇</p>

  <h3> ¿Qué hace <code>mergeCombine</code>?</h3>
  <pre><code class="language-scala">
@tailrec
private def mergeCombine(nodes: List[HuffmanNode]): HuffmanNode = nodes match {
  case node :: Nil => node
  case min1 :: min2 :: rest =>
    val merged = InternalNode(min1, min2)
    val updated = insertInOrder(rest, merged)
    mergeCombine(updated)
}
  </code></pre>

  <h4>🔁 Flujo:</h4>
  <ul>
    <li>Combina los dos nodos de menor frecuencia.</li>
    <li>Crea un nuevo nodo <code>merged</code>.</li>
    <li>Lo inserta en la lista usando <code>insertInOrder</code>.</li>
  </ul>
...
<div class="seccion derecha">
    <form action="./index.php" method="POST" onsubmit="handleSubmit(event)" autocomplete="off">
    <h3> Complejidad detallada</h3>
  <ol>
    <li><strong>symbols.map(...)</strong><br>
      Convierte <code>n</code> tuplas en nodos hoja.
      <br>🔹 <strong>O(n)</strong>
    </li>
    <li><strong>mergeCombine</strong><br>
      Hace <code>n - 1</code> combinaciones, una por cada nivel del árbol.<br>
      Cada combinación llama a:
      <br>→ <code>insertInOrder(rest, merged)</code><br>
      <br><em><code>insertInOrder</code> puede recorrer toda la lista para insertar ordenadamente.</em><br>
      En el peor caso: <strong>O(n)</strong> por inserción.
    </li>
  </ol>

  <p> Entonces:<br>
  Hay <strong>O(n)</strong> combinaciones<br>
  Cada una hace una inserción de <strong>O(n)</strong></p>

  <p> <strong>Complejidad total de <code>mergeCombine</code>:</strong> <code>O(n²)</code></p>
 

  <p> Si usas una estructura eficiente como <code>PriorityQueue</code>, podrías reducirlo a <strong>O(n log n)</strong>.</p>


  <h3>✅ Complejidad total de <code>build</code></h3>
  <table border="1" cellpadding="5">
    ...
    <tr>
      <td><code>symbols.map(...)</code></td>
      <td>O(n)</td>
    </tr>
    <tr>
      <td><code>mergeCombine(...)</code></td>
      <td>O(n²)</td>
    </tr>
    <tr>
      <td><strong>Total de <code>build(...)</code></strong></td>
