...
<div class="seccion izquierda">
    ...
    <h2>For para manejar facilmente secuencias anidadas</h2>
    <br><br>
    <strong>Un ejercicio que vimos que muestra como for reduce complejidad</strong>
    <img src="../../../img/secuencias.png" alt="" width="600">
    <br>
    <br>
    <img src="../../../img/FOR.png" alt="" width="600">
    <hr>
    <h2>📌 Objetivo del ejercicio</h2>
  <p>Generar todas las parejas <code>(i, j)</code> tales que <code>1 ≤ j &lt; i &lt; n</code> y <code>i + j</code> sea primo.</p>

  <h2>✅ Lo que haces en <code>generarSecuencia(n)</code></h2>
  <pre><code>((1 until n).toArray flatMap (
  j => (j + 1 until n).toArray.map(i => (i, j))
)
).toList filter (a => esPrimo(a._1 + a._2))</code></pre>

  <p>Esta expresión está haciendo lo siguiente:</p>
  <ul>
    <li><code>(1 until n)</code>: genera valores de <code>j</code>.</li>
    <li>Para cada <code>j</code>, se genera <code>(j + 1 until n)</code>: valores posibles de <code>i</code> tales que <code>i &gt; j</code>.</li>
    <li>Se mapea cada par <code>(i, j)</code> → genera todas las combinaciones donde <code>1 ≤ j &lt; i &lt; n</code>.</li>
    <li>Se filtra por la condición de que <code>i + j</code> sea primo.</li>
  </ul>

  <p>Esto es equivalente a usar dos ciclos anidados, pero estás usando:</p>
  <ul>
    <li><code>flatMap</code> (función de alto orden) para aplanar la secuencia de listas de tuplas.</li>
    <li><code>map</code> para construir las tuplas.</li>
    <li><code>filter</code> para aplicar la condición.</li>
  </ul>

  <p class="icon">✔️ Esto es exactamente lo que se pide: sustituir ciclos anidados con funciones de orden superior.</p>

  <h2>✅ Uso de for-comprehension</h2>
  ...
  <p>Aunque este estilo usa sintaxis de <code>for-comprehension</code>, Scala la transforma internamente en llamadas a <code>flatMap</code>, <code>map</code> y <code>filter</code>, por lo que también se considera una forma funcional/anidada válida.</p>

  <h2>🛠️ Mejora menor en legibilidad</h2>
  <p>Una versión más idiomática (usando colecciones en lugar de <code>toArray</code>) sería:</p>
  <pre><code>def generarSecuencia(n: Int): List[(Int, Int)] = {
  (1 until n).flatMap(j =>
    (j + 1 until n).map(i => (i, j))
  ).filter { case (i, j) => esPrimo(i + j) }.toList
}</code></pre>
    <hr>
    <h2>✅ ¿Por qué la solución con <code>for</code> es más sencilla?</h2>

<p>La solución con <strong>for-comprehension</strong>:</p>
...
<p>Es más sencilla por las siguientes razones:</p>

<div class="reason">
  <h3>🔹 1. Más legible y natural</h3>
  <p>El <code>for</code> expresa de forma explícita y ordenada la idea de recorrer dos variables anidadas (<code>j</code> y luego <code>i</code>), igual que lo harías con dos ciclos <code>for</code> tradicionales.</p>
  <p>Se puede leer como una frase en lenguaje natural:<br>
  <em>“Para cada j entre 1 y n, para cada i mayor que j hasta n, si i + j es primo, generar la pareja (i, j)”</em>.</p>
</div>
