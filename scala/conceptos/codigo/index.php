...
if ($_POST) {
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["codigo"])) {
        ...
        // Validación de la lista (ignorando espacios)
        $codigoSinEspacios = preg_replace('/\s+/', '', $codigo);
        $tieneListaCorrecta = str_contains($codigoSinEspacios, 'vall=1::2::3::Nil');
        ...
        $verificacionLista = $tieneListaCorrecta ? "correcto" : "incorrecto";
    }
    ...
    /*if (isset($_POST['codigo'])) {
        ...
        //cuando  queremos pasar el object
        //$codigoFinal = "object Main extends App {\n" . $codigo . "\n}";
        if (!str_contains($codigo, "object") && !str_contains($codigo, "@main")) {
            $codigoFinal = "@main def main() = {\n" . $codigo . "\n}";
        } else {
            $codigoFinal = $codigo;  // Mantener código tal cual si ya está en formato correcto
        }
        ...
        //version cuando queremos pasar el object
    ...
    if (isset($_POST['codigo2_2'])) {
        $codigo2_2 = $_POST["codigo2_2"];
    
        // Envolver el código en un objeto válido de Scala 3 si no tiene "object" o "@main"
        if (!str_contains($codigo2_2, "object") && !str_contains($codigo2_2, "@main")) {
            $codigoFinal2_2 = "object Main extends App {\n" . $codigo2_2 . "\n}";
        } else {
            $codigoFinal2_2 = $codigo2_2;  // Mantener código tal cual si ya está en formato correcto
        }
    
        // Guardar el código en un archivo temporal
        $archivoScala2_2 = "codigo2_2.scala";
        file_put_contents($archivoScala2_2, $codigoFinal2_2);
    
        usleep(100000);  
    
        // Ejecutar el código Scala y capturar la salida
        $salida2_2 = shell_exec("scala-cli run $archivoScala2_2 2>&1");
    
        // Filtrar salida para eliminar mensajes de compilación
        $lineas2_2 = explode("\n", $salida2_2);
        $salidaLimpia2_2 = [];
        foreach ($lineas2_2 as $linea2_2) {
            if (!str_contains($linea2_2, "Compiling project") && !str_contains($linea2_2, "Compiled project")) {
                $salidaLimpia2_2[] = $linea2_2;
            }
        }
    
        // Guardar la salida en una variable limpia
        $resultadoScala2_2 = implode("\n", $salidaLimpia2_2);
        $resultadoScala2_2_2 = trim(implode("\n", $salidaLimpia2_2));

        // Salida esperada para la validación
        $salida_esperada2_2 = "21";   
    
        if ((int)$resultadoScala2_2 === (int)$salida_esperada2_2) {  
            $verificar_2_2 = "correcto";
        } elseif ($resultadoScala2_2 === '') {
            $verificar_2_2 = 'sin salida';
        } else {
            $verificar_2_2 = "incorrecto";
        }
...
<div class="seccion izquierda">
    ...
    Escribir HolaMundo en scala <br><br>

    declaracion de objeto HolaMundo:
    ...
    <br><br>
    impresion: 
    <input type="text" name="respuesta_3" value="<?php echo $respuesta_3; ?>">("!Hola Mundo")
    } <br><br>
    }
    <br><br>
    ...
    <strong>Declarar Lista sin List(..) de los 3 primeros numeros</strong>
<br><br>
<textarea id="codigo" name="codigo" rows="4" cols="50"><?php echo htmlspecialchars($codigo); ?></textarea>
<br>
<button onclick="ejecutarCodigo()">Validar y Ejecutar</button>

<?php if(isset($verificar_lista)): ?>
    <p>Validación de lista: 
        <span style="color: <?php echo $verificar_lista === 'correcto' ? 'green' : 'red'; ?>">
            <?php echo $verificar_lista; ?>
        </span>
    </p>
<?php endif; ?>

<p>Resultado:</p>
<pre id="resultado"><?php echo $salida; ?></pre>
<?php echo $verificacionLista ?>
<script>
...
async function ejecutarCodigo2() {
    ...
    <hr>
    <strong>Declarar un metodo que siempre que lo llame me devuelva una nueva instancia una lista "numeros"
    (que contiene: 1,2,3,4,5): </strong>
    <br><br>
    (no definir el objeto, sino funciones y demas)
    <br><br>
    <textarea id="codigo2" name="codigo2" rows="10" cols="50" value="<?php echo $codigo2; ?>" ><?php echo htmlspecialchars($codigo2); ?></textarea>
    <br>
    <button onclick="ejecutarCodigo2()">Ejecutar</button>
    <?php echo $verificar_2_1 ?>
    <p>Resultado:</p>
    <pre id="resultado2"></pre>
    <?php echo $salida2 ?>
    <?php if(isset($verificacionLista)): ?>
    <p>Validación de lista: <strong><?php echo $verificacionLista; ?></strong></p>
    <?php endif; ?>
 ...
 async function ejecutarCodigo2_2() {
    ...
    <strong>Implementar la funcion sumaRecursionLineal </strong>
    (suma los primeros 6 numeros)
    <br><br>
