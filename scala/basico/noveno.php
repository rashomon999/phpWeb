...
<div class="seccion izquierda">
    ...
    <strong>Analizando la expresion: replaceAll("[^0-9,]", "")</strong>
    <br><br>
    El ^ dentro de los corchetes [^...] en una expresión regular (regex) significa "negación" o "todo lo que NO sea".
    <br><br>
    "[^0-9,]" <br><br>
    Significa:
    "Selecciona cualquier carácter que NO sea un número (0-9) o una coma (,)."
    <hr>
    <strong>Ejemplo practico: </strong>
    <code>val input = "[(1,2),(3,4)]"  
    <br>
    val cleaned = input.replaceAll("[^0-9,]", "") 
    <br>
    println(cleaned)</code>
    <br>
    <strong>salida: "1,2,3,4"</strong>

    <hr>
   
    <h2>Ejemplo: Función suma parametrizada en f</h2>
    <pre>
    ...
    <h2>Explicación</h2>
    <p>El tipo A => B representa funciones que reciben valores de tipo A como argumento y devuelven valores de tipo B como resultado.</p>
    
    <h3>Ejemplos de Tipos de Funciones</h3>
    <ul>
        <li>Int => Int: Funciones que reciben un Int y devuelven un Int.</li>
        <li>(Int, Int) => Boolean: Funciones que reciben dos valores Int y devuelven un Boolean.</li>
    </ul>

    <hr>
    <h2>Funciones anonimas: </h2>
    <h4>¿Es necesario nombrar todas las funciones?</h4>
    <p>Usar funciones como parámetro nos ha llevado a crear (nombrar) muchas funciones pequeñas nuevas, como <strong>suc</strong>, <strong>sum2</strong>, <strong>ident</strong> y <strong>cuadrado</strong>.</p>
    
    <p>Miremos lo que sucede con otros tipos de datos, como las cadenas de texto. No es necesario definir una cadena antes de imprimirla.</p>
    
    <h4>Ejemplo con variables</h4>
    <pre>
    def cad = "abc" 
    println(cad)
    </pre>
    
    <h4>Ejemplo sin definir previamente la variable (Si hay varios par´ametros, se separan por comas:)</h4>
    <pre>
    println("abc")
    </pre>
    
    <p>En este caso, las cadenas pueden existir como literales, sin necesidad de una definición previa.</p>

    <p>Es posible escribir literales de funciones, permitiendo definirlas sin necesidad de un nombre. Esto se conoce como <strong>funciones anónimas</strong>.</p>
    
    <h2>Ejemplo de función anónima para elevar al cubo</h2>
    <pre>
    (x: Int) => x * x * x
    </pre>
    
    <p>Aquí, <code>(x: Int)</code> es el parámetro de la función y <code>x * x * x</code> es su cuerpo. El tipo del parámetro puede omitirse si el compilador puede inferirlo.</p>
    
    <h2>Ejemplo con múltiples parámetros</h2>
    <pre>
    (x: Int, y: Int) => (x + y) / 2
    </pre>
...
<div class="seccion derecha">
    <form action="./noveno.php" method="POST" onsubmit="handleSubmit(event)" autocomplete="off">
    <h2>Ejemplo de uso en la función suma</h2>
    <pre>
    def sumaEnteros2(a: Int, b: Int): Int =
    suma(x => x, x => x + 1, a, b)
    ...
    <pre>
    def sumaCuadrados2(a: Int, b: Int): Int =
    suma(x => x * x, x => x + 1, a, b)

    sumaCuadrados2(1, 10)
    </pre>

    <pre>
    def sumaAlternada2(a: Int, b: Int): Int =
    suma(x => x, x => x + 2, a, b)

    sumaAlternada2(1, 10)
    </pre>
    <hr>
    <strong>Mas sobre split: </strong>
  
    <h2>La expresión split("\\s*,\\s*")</h2>
    <p>Se usa para dividir una cadena en partes usando comas como separador, pero permitiendo
    espacios opcionales antes y después de la coma.</p>
    <h3>Explicación detallada</h3>
    <li>
    \\s significa espacio en blanco (como espacios, tabulaciones o saltos de línea).
    </li>
    <li>
    Significa cero o más repeticiones de lo anterior.
    </li>
    <li>
    Indica que se debe dividir por la coma real.
    </li>
    <p>En conjunto, "\\s*,\\s*" significa:</p>
    "Dividir en comas, pero ignorando espacios antes o después de la coma".
    <hr>
    <h2>Conversión de Listas en Scala</h2>
    <p>Sí, en Scala puedes convertir una lista de enteros a una lista de cadenas y viceversa usando <code>.map</code> y <code>.toInt</code>.</p>
    
    <h3>1️ Convertir una Lista de Enteros → Lista de Strings</h3>
    <pre><code>val listaEnteros = List(1, 2, 3, 4, 5)
    val listaStrings = listaEnteros.map(_.toString)

    println(listaStrings) // List("1", "2", "3", "4", "5")</code></pre>
    
    <h3>2️ Convertir una Lista de Strings → Lista de Enteros</h3>
    <pre><code>val listaStrings = List("1", "2", "3", "4", "5")
    val listaEnteros = listaStrings.map(_.toInt)

    println(listaEnteros) // List(1, 2, 3, 4, 5)</code></pre>
    
    <h3> Manejo de valores no numéricos</h3>
    <p>Si la lista de strings puede contener valores no numéricos, usa <code>try</code> para evitar errores:</p>
    <pre><code>val listaStrings = List("1", "2", "a", "4")
    val listaEnteros = listaStrings.flatMap(s => s.toIntOption)

    println(listaEnteros) // List(1, 2, 4) (ignora "a" porque no es un número)</code></pre>
    </form>
</div>
