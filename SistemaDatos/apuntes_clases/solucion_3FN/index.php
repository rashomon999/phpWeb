...
<div class="seccion izquierda">
    ...
    <hr>
    <strong>Nota:</strong>
    (se debe escribir el ejercicio, primero, estas son algunas claves):
    <br><br>
    <h3>✅ ¿Por qué N → I, N → E?</h3>
  <p>Estas dependencias significan:</p>
  <blockquote>
    “Dado el nombre de un investigador (<strong>N</strong>), puedo conocer exactamente su número de años de experiencia (<strong>I</strong>) y su edad (<strong>E</strong>).”
  </blockquote>

  <h4>📌 Fundamento: el enunciado dice</h4>
  <blockquote>
    “Suponga que no hay dos investigadores con el mismo nombre”.
  </blockquote>

  <p>Esto implica que:</p>
  <ul>
    <li><strong>N</strong> (el nombre del investigador) identifica de forma única a cada persona.</li>
    <li>Por tanto, <strong>ningún nombre se repite</strong>.</li>
    <li>Entonces, a cada <strong>N</strong> le corresponde una sola edad y una sola cantidad de años de experiencia.</li>
  </ul>
  <hr>
  <h3>📌 Análisis de la dependencia N → T</h3>
  <p>
    Una dependencia funcional <strong>N → T</strong> significa que, dado un valor de <strong>N</strong>, existe un único valor asociado de <strong>T</strong>.
    Sin embargo, si un investigador puede tener múltiples títulos (por ejemplo, Licenciatura, Maestría, Doctorado), entonces <strong>T</strong> es un atributo multivaluado.
    En este caso, la dependencia funcional <strong>no se cumple</strong> porque un solo valor de <strong>N</strong> se asocia con múltiples valores de <strong>T</strong>.
  </p>

  <h4>🧩 Modelado adecuado</h4>
  <p>
    Dado que <strong>T</strong> es multivaluado, la mejor práctica es modelar los títulos como una entidad separada relacionada con el investigador.
    Esto se representaría como una relación <strong>uno a muchos</strong> entre <strong>INVESTIGADOR</strong> y <strong>TITULO</strong>, donde cada título está asociado a un único investigador,
    pero un investigador puede tener múltiples títulos.
  </p>

  <h4>✅ Conclusión</h4>
  <p>
    En resumen, aunque <strong>N</strong> es un identificador único para los investigadores, <strong>no se puede afirmar que N → T</strong> si <strong>T</strong> es un atributo multivaluado.
    Por lo tanto, es más apropiado modelar los títulos como una entidad separada relacionada con los investigadores.
  </p>

  <hr>
  <strong>Relación entre investigador, proyecto y función:</strong>
    <ul>El texto dice:
        <li> “Cada investigador en un proyecto tiene asignada una función”
        </li>
    </ul>
    <strong>que identifica al investigador, el nombre, por eso N (nos dicen que no hay dos con el mismo nombre)</strong><br><br>
    <p>Eso implica que la función (F) está determinada por la combinación N, P.
    </p>
    
    <code>N, P → F</code>

    <p>Es decir: para cada combinación de investigador y proyecto, hay una única función asignada.
    </p>

    <hr>
    <strong>“Dependiendo de la función desempeñada y del beneficio económico (B) que aporta el 
...
Esta frase nos dice que el porcentaje que recibe un investigador (%) está determinado por dos cosas:
    ...
    <li>El beneficio económico del proyecto (B)
    </li>
</ul> 

    <p>✅ Dependencia funcional que surge:
    <code>F,B→%</code></p>

    <strong>    Esto significa: 
        </strong>
         <p>Dada una combinación de función (F) y beneficio económico (B), se asigna un único valor de porcentaje (%) al investigador.
        </p>


    <hr>
    <strong>Analisis del fragmento fragmento:</strong>
    <p>"Cada proyecto pertenece al ámbito de una o más áreas científicas (A); y se caracteriza
    por un determinado estado (X) (concluido, en curso, suspendido)."
    </p>
    <strong>🟨 De aquí surge:</strong>
    <ul>
    1)Sobre el proyecto (P) y su estado (X):
    <li>El estado es una sola cosa por proyecto, es un atributo simple.
    </li>
    <li>Entonces:</li>
    <code>P→X</code>
    <li>2)Sobre proyecto y áreas científicas (A):
    </li>
...
<div class="conclusion">
  ...
  📌 <code>P → B</code> porque <strong>B</strong> es un atributo propio del proyecto.
    ...
    <h4>Texto clave:</h4>
    <p>
    “Cada proyecto posee una duración programada, idéntica para todos los proyectos, 
    y que está asociada a una fecha de inicio programada (fi) y fecha de terminación programada (ft).”</p>
 
    <div>
  <h3>📌 Dependencias funcionales que surgen del fragmento:</h3>

  <ol>
    <li>
      <strong>Entre fechas programadas:</strong><br>
      <code>fi → ft</code><br>
      <code>ft → fi</code><br>
      <em>Esto refleja que una fecha determina la otra (porque la duración programada es la misma para todos y por tanto conocida).</em>
    </li>

    <li>
      <strong>Desde el proyecto:</strong><br>
      <code>P → fi, ft</code><br>
      <em>Porque cada proyecto tiene una sola fecha de inicio programada y una sola de terminación programada.</em>
    </li>
  </ol>

    <h4>🔁 Relación entre ellas:</h4>
    <p>
    Estas tres dependencias son <strong>coherentes entre sí</strong> y <strong>no se contradicen</strong>:<br>
    <code>P → fi</code> y <code>fi → ft</code><br>
    ⟶ Implica también que <code>P → ft</code>, lo cual es consistente.
    </p>

    <h4>✅ Forma final (resumen):</h4>
    <pre>
    fi → ft
    ft → fi
    P → fi, ft
    ...
    <strong>Clave: pensar las cosas de forma: para cada fecha inicial hay fecha final y viceversa</strong>
    <br><br>
    <strong> sobre lo anterior hay una dependencia, porque nos dicen que cada proyecto tiene una duracion
    </strong>
    <br><br>
    parte clave: "duracion.... y que esta asociada...."
    <hr>

    <h4>Texto clave: </h4>
    SIN EMBARGO, UNA COSA SON LAS FECHAS PROGRAMADAS Y OTRA SON LAS FECHAS REALES DE INICIO (Fi) Y TERMINACIÓN (Ft).

    <h3>📌 Fechas reales (<code>Fi</code>, <code>Ft</code>)</h3>
    <p>
    Las fechas reales varían por proyecto y <strong>no se calculan automáticamente</strong>.
    </p>
    <p>
    Cada proyecto puede haber empezado o terminado en una fecha distinta a la programada.
    </p>
    <p>
    Por tanto, estas fechas <strong>dependen directamente del proyecto</strong>, no de <code>fi</code> o <code>ft</code>.
    </p>

    <h4>🔗 Dependencias funcionales:</h4>
    <pre>
    P → Fi, Ft
    </pre>
    <p><em>(Cada proyecto tiene una sola fecha real de inicio y una sola de fin)</em></p>
    </div>
    <hr>
   </form>
</div>
...
<div class="seccion derecha">
    <form action="./index.php" method="POST" onsubmit="handleSubmit(event)" autocomplete="off">
    <p><strong>Fi no deriva Ft.</strong></p>

    <p>Te lo resumo:</p>

    <p>Las fechas reales de inicio (<strong>Fi</strong>) y de terminación (<strong>Ft</strong>) pueden variar
    independientemente, porque no hay una regla fija ni duración constante para las fechas reales.</p>

    <p>Por eso, no existe una dependencia funcional <strong>Fi → Ft</strong> ni <strong>Ft → Fi</strong>.</p>

    <p>Por lo tanto, ambas deben depender directamente del proyecto (<strong>P → Fi</strong> y <strong>P → Ft</strong>)
    y no pueden derivarse una de la otra.</p>

    <strong>y una dependencia funcional precisamente se llama asi porque algo depende de otro.</strong>
    <hr>
    ej: una asignatura posee uno o mas grupos.   ❌. no es porque no hay un único grupo por asignatura, sino varios.
    <br><br>
    ej: una asignatura posee uno o mas profesores, pero solo uno por grupo. ✅ grupo es lo que hace que un profesor sea unico, 
    por eso: "posee muchos pero solo uno cuando....". AG -> P
    <br><br>
    ej: Un profesor imparte clases en una o mas asignaturas, pero solo en un grupo por asignatura. ✅ PA->G ,asignatura es lo 
    que hace al profesor unico. 
    <br><br>
