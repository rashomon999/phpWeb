...
$respuesta_1_1 = 'cod-prov'; $respuesta_1_2 = 'nom-p'; $respuesta_1_3 = 'ciudad-p';  
...
<style>
    ...
    /* Contenedor único que reemplaza a .seccion y .centered-container */
.container {
    width: 90%; /* Puedes ajustar este valor según tu necesidad */
...
<div class="container">
    ...
    <strong>Una condicion implica \( \sigma \) y una nueva union de entidad</strong> <br><br>
    <strong> Un Alpha Join (o Natural Join o Equi Join cuando se usa con condiciones de igualdad) 
        es muy similar a un producto cartesiano seguido de una selección (sigma).</strong>
    <br><br>
    <strong> 1)obtener los codigos y los nombres de los articulos:</strong>
    $$$$
    <br>
    \[
    \pi_{\text{id-art}, \text{nom-art}} \left( \text{ARTICULO} \right)
    \]
    $$$$
    <br>
    <strong>2)Obtener los nombres de las fabricas abastecidas por los proveedores de cali que suministran
    articulos de color azul</strong>
    <br><br>
    \[
    \pi_{\text{nom-fab}} \left( \sigma_{\text{ciudad} = \text{'Cali'} \land \text{color} = \text{'azul'}} 
    \left( \text{PROVEEDOR} \bowtie \text{PEDIDO} \bowtie \text{ARTICULO} \bowtie \text{FABRICA} \right) \right)
    \]
    $$$$
    <br>
    <strong>3)obtener los nombres de los proveedores que suministran articulos de color rojo.</strong>
    \[
    \pi_{\text{nom-prov}} \left( \sigma_{\text{color} = \text{'rojo'}} \left( \text{PROVEEDOR} \bowtie \text{PEDIDO} 
    \bowtie \text{ARTICULO} \right) \right)    
    \]
    $$$$
    <br>
    <strong>significado 3:</strong> El JOIN entre las tres tablas (PROVEEDOR, PEDIDO y ARTICULO) implica que solo los proveedores
    que tienen una relación con algún pedido de algún artículo (es decir, los que han suministrado al menos un artículo) 
    estarán presentes en el resultado.
    $$$$
    <br>
    <strong>4)obterner los nombre de los articulos suministrados en cantidades mayores a 100.</strong>
    \[
    \pi_{\text{nom-art}} \left( \sigma_{\text{cantidad} > 100} \left( \text{PEDIDO} \bowtie \text{ARTICULO} \right) \right)    
    \]
    $$$$
    <br>
    <strong>5)obtener los nombres de los proveedores que suministran el articulo de codigo 'Ab'</strong>
    \[
    \pi_{\text{nom-prov}} \left( \pi_{\text{cod-prov}} \left( \sigma_{\text{cod-art} = 'Ab'} \left( \text{PEDIDO} \right) \right) \Join \text{PROVEEDOR} \right)
    \]
    $$$$
    <br>
    <strong>6)obtener los nombres de las fabricas para las que hay un proveedor en su misma ciudad. realice la consulta sin exigir
    que el proveedor provea la fabrica </strong>
             

    \[
    \pi_{\text{nom-fab}} \left( \text{PROVEEDOR} \bowtie_{\text{ciudad-prov} = \text{ciudad-fab}} \text{FABRICA} \right)
    \]
    <br><br>
    <strong>7)obtener los codigos de los provedores diferentes de 03 que me suministran al menos todos los articulos
    suministrados por 03</strong>
    <br><br>
    \[ \pi_{\text{cod-prov}} \left( \sigma_{\text{cod-prov} \neq '03'} (\text{PEDIDO}) \div \pi_{\text{cod-art}}
     (\sigma_{\text{cod-prov} = '03'} (\text{PEDIDO})) \right)
    \]
    <br><br>
    <strong>8)obtener los nombres de los proveedores de fuera de cali que suministran algun articulo suministrado
    por un proveedor de cali, pero a un precio menor</strong>   
    $$$$
    <br>
    \[
    \pi_{\text{nom-prov}} \left( 
    \left( \sigma_{\text{ciudad} \neq \text{'Cali'}} (\text{PROVEEDOR}) \bowtie \text{PEDIDO} \right) 
    \bowtie_{\text{cod-art} = \text{cod-art} \land \text{precio} < \text{precio}} 
    \left( \sigma_{\text{ciudad} = \text{'Cali'}} (\text{PROVEEDOR}) \bowtie \text{PEDIDO} \right) 
    \right)
    \]
    ...
    <strong>clave:</strong>
    <p><strong>Hacer un JOIN con la tabla PROVEEDOR:</strong></p>
    <p>En este paso, haces un JOIN entre la tabla de proveedores (PROVEEDOR) y la tabla que contiene
    solo los códigos de proveedores obtenidos en el paso anterior.</p>
    <p>El JOIN conecta las filas que tienen los mismos valores en el campo <strong>cod-prov</strong> (es decir,
    el código del proveedor). Este es el paso clave donde se filtran los proveedores que no están en la lista 
    obtenida en el paso anterior.</p>

    <p><strong>Obtener los nombres de los proveedores:</strong></p>
    <p>Finalmente, se realiza una proyección <span style="font-family: 'Courier New', monospace;">π<sub>nom-prov</sub></span>
    para obtener solo los nombres de los proveedores (<strong>nom-prov</strong>) de los resultados del JOIN.</p>
    <p>Solo los proveedores que suministran el artículo 'Ab' estarán presentes en el resultado final.</p>
...
<table border="1">
  ...
  <tbody>
    <tr>
      ...
      <td><input type="text"  size="8"></td>
      <td><input type="text"  size="8"></td>
      <td><input type="number"  size="8"></td>
    </tr>
    <tr>
      <td><input type="text"  size="8"></td>
      <td><input type="text"  size="8"></td>
