...
<div class="seccion izquierda">
    ...
    <h3>¿Cuándo SÍ usar DISTINCT?</h3>
<ul>
  <li>Solo te interesa saber quiénes son jefes (sin importar cuántos subordinados tengan)</li>
  <li>Quieres eliminar repeticiones del mismo jefe</li>
  <li>Estás buscando valores únicos</li>
</ul>

<p>Ejemplo corregido:</p>
<pre>
SELECT DISTINCT j.empno, j.ename
...
<h3>¿Cuándo NO usar DISTINCT?</h3>
<ul>
  <li>Necesitas contar cuántos empleados tiene cada jefe</li>
  <li>Quieres ver todas las relaciones jefe-empleado</li>
  <li>Estás trabajando con datos agregados (GROUP BY ya maneja la unicidad)</li>
</ul>

<p>Ejemplo sin DISTINCT:</p>
<pre>
SELECT j.ename, COUNT(*) AS empleados_a_cargo
FROM EMP e JOIN EMP j ON e.mgr = j.empno
...
<ul>
  <li>Primero prueba sin <code>DISTINCT</code> para entender la estructura de los datos</li>
  <li>Agrega <code>DISTINCT</code> solo si ves duplicados que no aportan información</li>
  <li>En self-joins jerárquicos, casi siempre necesitarás <code>DISTINCT</code> para listados de jefes</li>
</ul>

<h3>Alternativa: EXISTS</h3>
<p>Puedes usar <code>EXISTS</code> en lugar de <code>JOIN</code> si solo necesitas verificar la existencia:</p>
<pre>
SELECT empno, ename
FROM EMP j
WHERE EXISTS (SELECT 1 FROM EMP e WHERE e.mgr = j.empno);
</pre>
    <hr>
    <strong>Una clave que determina el grado es que grade depende de low y hight, es decir con solo saber si el 
    salario entra en este intervalo, estamos sabiendo grade (que se supone (logica) que se determina por esto).
    </strong>
    </form>
</div>
